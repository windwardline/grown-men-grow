...
**`docs/technical/substack-notes.md` is now that record, and `scripts/verify-substack-notes.mjs` reconciles it against the live public profile.** Substack has no documented API, but the notes feed is public and answers unauthenticated — `api/v1/reader/feed/profile/<id>` returns each note's id, exact UTC timestamp, and **full body text**. So unlike the Ghost checker, which needs a Keychain key and is therefore `cadence:`, this one needs no credential and runs in CI on every pull request as the `Substack notes register` job.
...
**Two things in this change set could not be completed and are not quietly omitted.** `scripts/verify-required-checks.mjs` requires every job that runs on a pull request to be a required status context. The `Substack notes register` job therefore needs adding to the `main-requires-green-ci` ruleset **in this same change set**, and the ruleset write was refused by the Claude Code auto mode classifier, as was running `verify-required-checks.mjs` itself, which reads the same API. No substitution was attempted. **This pull request will fail its own `Required checks` job until the founder adds the context**, and that is the contract working rather than failing: the job exists precisely so an ungated job cannot land unnoticed. The exact command is in the pull request body.
