...
**Client:** Claude Code (desktop), the `gmg-saturday-draft` scheduled task, founder not present. **Branch:** `claude/field-note-18-draft-2026-09-05`. **PR:** #161, auto-merge armed.
...
**Concurrency, recorded because it nearly went wrong.** This session began on branch `claude/saturday-note-2026-09-05` with a neighbouring session's handoff entry staged in the shared index. No work was committed while that was true. The neighbour merged as #160 and its cleanup left the shared checkout on `main`, so a branch was created before anything was committed, and only explicit paths were staged throughout — never `git add -A`. Auto-merge on #161 was armed before this entry was written, which is the wrong order; the standing lesson is to arm it last, and it is recorded here rather than quietly corrected.
