...
## 2026-08-26 — Claude Code: Field Note 15 drafted complete; imagery type-led because Chrome was down

**Client:** Claude Code (scheduled task `gmg-wednesday-draft`, unattended). **Branch:** `claude/field-note-15-draft-2026-08-26`.

**Backfill first, and it was already clean.** Every one of the thirteen approved notes was checked against the complete-unit table in `content-pipeline.md` before any new writing: packs 02–14 present, render scripts 02–14 present, thirteen carousels, sixteen feature images, thirteen review sheets, and matching gate entries in both directions. Nothing was outstanding, so the run proceeded to the new draft rather than stopping at a backfill.

**Field Note 15 — "Every Part Passed Inspection", 1,231 words, stance `assignment`.** Written to `drafts/`, not `content/`. Frontmatter records it as not founder-approved and not authorized for publication, newsletter, posting, or scheduling. The subject is tolerance stack-up: six parts each inside spec, all drifting one direction, an assembly thirty thousandths out, and no defective part in it. The argument separates fault from responsibility — an inspection that finishes at *no defect found* versus a gauge that reads the same number whichever way blame falls. No subject, argument, structural move, or opening gambit repeats from the thirteen banked notes or Essay 1.

**Register held.** Generic present throughout, no first person, no asserted event, no composite. The `Datum` section deliberately refuses to name the reader's reference face, which is where this piece would otherwise have prescribed.

**The imagery is type-led, and that is a ruling being followed rather than a step skipped.** `mcp__claude-in-chrome__tabs_context_mcp` returned not-connected on both attempts, so no ChatGPT session could be opened and no source photography exists for this note. The 2026-08-16 photograph-exclusivity ruling forbids re-cropping any photograph already spent on another asset, and `editorial-visual-system.md` names a type-led `Marginalia` composition as the answer in precisely this case. The seven slides and the feature image are therefore typographic, built on a new signature mark — a tolerance stack of measured segments walking away from a ruled datum, resolving to one datum on the close. `render-field-note-15.mjs` throws if the composition references any `/assets/source/` path, so the type-led choice is enforced rather than asserted. Three house-shape prompts and their reserved source paths are recorded in the note's production notes, so an image-led rebuild is one paste away if the founder prefers it.

**Four rendering defects were found by looking at the output and fixed, not shipped.** A background band painted over the slide-1 header; serif copy at reading sizes ran words together under the `-1.6` display tracking default across five slides; slide 5's italic line collided with the descenders above it; and the first feature composition ran the stack into the oxblood field with an unmotivated black block. Slide 4's background mark was also repositioned after it proved invisible behind its card. Each was confirmed by re-reading the rendered PNG.

**Files changed:** `drafts/every-part-passed-inspection.md`, `content/distribution/field-note-15-platforms.md`, `scripts/render-field-note-15.mjs`, `scripts/render-review-contact-sheets.mjs`, `scripts/verify-repository.mjs`, sixteen rendered SVG/PNG assets under `assets/drafts/`, and this log.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus `gscan -z --fatal --verbose dist/grown-men-grow.zip`, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (127 pass, 0 fail); `node scripts/verify-repository.mjs` (536 tracked files, 44 JavaScript files, 147 PNG/SVG pairs, 19 review sheets); `bash scripts/verify-svg-xml.sh` (158 SVG files); `git diff --cached --check` clean after fixing an indent-only line the renderer emitted when a mark's gap bracket was suppressed; staged file list reviewed and confined to this work.
...
## 2026-08-26 (fourth entry) — Claude Code: the four open items closed

**Client:** Claude Code (continuation, founder present). **Branch:** `claude/excerpt-defect-2026-08-26`.

The founder authorised executing the standing recommendations on all four open items. All four are closed. Two of them turned out to be misdescribed, and both corrections are the substantive part of this entry.

**The excerpt item was not a founder call. It was a builder defect, live.** It had sat open since 2026-08-24 as "which of the two fields belongs in the excerpt." `theme/post.hbs` renders `custom_excerpt` as `article-header__deck` — the visible subtitle under the title — so the theme had already answered it. The builder sent `front.preview`, the email preheader line, which means every post it staged printed the inbox copy as its on-page subtitle. Verified live: `a-confession-can-still-be-selfish` rendered *"Disclosure has a feel-better exit. Repair does not."* as its deck while its approved dek sat in `meta_description` where no reader sees it. Field Note 2 was correct only because it was hand-staged before the builder existed, which is why the two disagreed and the divergence report kept surfacing it as taste. Fixed in the builder, guarded by a regression test proved to fail on the old behaviour, and corrected on the one affected live post — only `custom_excerpt` sent, with status, `published_at`, title, `meta_description` and `email_subject` verified unchanged afterwards and both live pages confirmed rendering their approved dek. The tradeoff is recorded rather than buried: Ghost has one excerpt field and also uses it as the newsletter preheader, so `preview` now has no Ghost destination and is an internal field only.

**The build-metaphor run was five, not four, and my own recommendation had the miscount in it.** The register recorded four consecutive build-trade openings at 11–14 and offered `the-lights-never-flickered` as one of the pieces available to break them. It opens on anchor bolts set an inch and a half off a template. It was part of the run, not a separator, and the run was five. Five build pieces need four separators; the bank offers three, which takes the longest run from five to two — the best this bank allows. `your-body-keeps-the-books` moved 7 → 12 to supply the third separator; it is the only constraint-bearing piece touched and it moved later, which its constraint permits. Nothing scheduled moved. Each of the three binding constraints was broken deliberately and confirmed to fail by name before being restored.

**The stance commission is a marker, not a note to the next agent.** `operating-cadence.md` carries `**Standing commission:** witness`, asserted by the checker. The corpus measurement found witness at two pieces in fifteen, and the subject test does not close that gap on its own: every draft with a move available reads as an assignment and almost every subject has a move. It is an instruction about what to look for, never a licence to force a subject through the three witness gates — a draft task that cannot find a passing subject leaves the commission standing rather than shipping an assignment piece wearing the label, and clears it to `none` in the change set that lands a witness note.

**The A/B is suspended with its precondition stated.** The 2026-08-23 readout found the list at one member and correctly called the distribution problem the real one, but made no protocol change. A protocol specifying an experiment it cannot run — while step 4 says "minimum four sends per arm" as though four sufficed at any size — reads as an experiment in progress and produces noise that reads as findings. `**A/B minimum recipients:** 30` now gates steps 3 and 4. Thirty is the floor at which the protocol's own five-point decision threshold can resolve, not a confidence target. Nothing here touches distribution.

**Files changed:** `scripts/lib/field-note-post.mjs`, `scripts/test/field-note-post.test.mjs`, `scripts/verify-repository.mjs`, `docs/technical/publication-order.md`, `docs/technical/publish-timing.md`, `docs/technical/operating-cadence.md`, `docs/technical/decision-log.md`, `AGENTS.md`, and this log.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test` and packaged-zip fatal GScan, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail — one added); `node scripts/verify-repository.mjs` (539 tracked files); `bash scripts/verify-svg-xml.sh` (158 SVG files); `git diff --cached --check` clean. Twelve gate failure modes proved to fire by name across this run — two register constraints, five markers, and five corpus checks — with the tree restored byte-exact after every probe.
...
## 2026-08-30 (second entry) — Claude Code: the Thursday Medium slot executed three days late

**Client:** Claude Code (scheduled task `gmg-thursday-medium`, founder not present). **Branch:** `claude/thursday-medium-import-2026-08-30`.

**The story is live on Medium:** https://grownmengrow.medium.com/a-confession-can-still-be-selfish-82984a7a161c

**The slot fired on Sunday evening, not Thursday morning, and this task has no lateness guard.** The run began at 18:36 EDT on 2026-08-30 against a 06:45 ET Thursday slot — the same catch-up burst the entry above documents for `gmg-saturday-note`, which stood down at 18:36:44 the same minute. The note task refused its slot because a note posted nine hours late costs the baseline it is measured against. The Medium import has no equivalent guard and needs none: its binding constraint is the canonical-first rule, not the clock. The essay published 2026-08-25 12:00 UTC, 130.6 hours before the run against a 40-hour minimum, and a syndicated copy carries no time-of-day signal the way a social post does. Being late here means the slot was **missed on Thursday**, not that executing it now is wrong. Recorded plainly because the handoff log should not let a three-day gap read as an on-time run.

**Preconditions, each checked rather than assumed.** `latestPublishedPost()` returned `a-confession-can-still-be-selfish`, status `published`, `published_at` 2026-08-25T12:00:00Z. `https://grownmengrow.com/a-confession-can-still-be-selfish/` returned 200. Neither grownmengrow.medium.com nor Stories → Drafts held this piece — the profile listed only *Call Your Friends Before There's a Reason* (Aug 18) and *Strength Has to Grow Up* (Aug 9), and drafts read "No stories in draft." Both surfaces were checked because the profile feed lists published stories only and a stale import draft is invisible to it.

**The importer got the same three things wrong the task file predicts, and a fourth that is worth recording.** The title imported as *A Confession Can Still Be Selfish | Grown Men Grow* — Medium reads the page `<title>`, and Ghost appends the site suffix even though this post's `meta_title` is null. Replaced with the post's `title` field. The `Grown Men Grow 2 min read` line lifted from the Ghost article header was removed. The preview image imported empty despite the artwork landing correctly in the body, and was set from the story's feature image through the publish dialog.

**The fourth is a correction to the task file's tag procedure, and it silently produces wrong tags.** The file says to enter tags one per tool call because batching type+Return "commits nothing." That is not what happens. Typing `Men` and pressing Return committed **Mental Health** — Medium's combobox commits the first autocomplete suggestion, not the typed string, and the first suggestion for a short tag is frequently a longer unrelated one. The failure is silent: a chip appears, so the step looks successful. The reliable method is to type the tag, press Down until the exact match is highlighted (verified on screen — `Men` was the third suggestion, behind *Mental Health* and *Mental Health Awareness*), then Return. Clicking the suggestion with the mouse does not work either; the list closes on mousedown and no chip is added. `Mental Health` was removed before publication and never went live. The five tags on the published story are exactly the pack's: Men, Relationships, Honesty, Masculinity, Personal Growth.

**One editing mistake was made, caught, and undone before it reached anything public.** Removing the min-read line with End → Shift+Home → BackSpace destroyed the title and subtitle as well: `Home` in Medium's contenteditable moves to the **document** start, not the line start, so Shift+Home selected everything above the cursor. The damage was found by reading the document structure rather than the screenshot — `.graf--title` and `.graf--subtitle` were absent from the DOM while the viewport still looked plausible. One `cmd+z` restored the tree with the corrected title intact, and the line was then removed by counting its 25 characters. Nothing was published in the broken state.

**Verified after publication, from server-delivered source rather than the rendered DOM.** `fetch` inside the page context returned 200 and `<link rel="canonical" href="https://grownmengrow.com/a-confession-can-still-be-selfish/"/>`. Run in the browser, never with `curl`, which Medium answers 403 — an unread exit status there turns a refused request into a false pass. Before publishing, the stored canonical was also confirmed in Advanced Settings: the field is disabled and its placeholder reads `Type the canonical URL...`, so the greyed `https://grownmengrow.com/a-confession-can-still-be-selfish/` in it was a real stored value. The live page's `h1` reads *A Confession Can Still Be Selfish* and the imported min-read line is absent.

**The post-publish share controls were not touched.** Medium's dialog offers Facebook, LinkedIn, and X; the distribution boundary gives other platforms reviewed native copy, never a Medium reshare. The dialog was dismissed with its close control.

**Files changed:** this log only. No content, script, gate, or configuration change.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus fatal GScan on the packaged zip, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail); `node scripts/verify-repository.mjs` (539 tracked files, 44 JavaScript files); `bash scripts/verify-svg-xml.sh` (158 SVG files); `git diff --cached --check` clean; staged file list reviewed and confined to this log.

**A concurrent session holds this checkout.** `HEAD` had moved from `3e4c0a2` to `565c9c8` on branch `claude/saturday-note-standdown-2026-08-30` between the start of this run and the commit, and two untracked drafts (`drafts/a-temp-wall-comes-out-in-april.md`, `drafts/field-note-16-platforms.md`) belong to another session in flight. This branch was cut from `origin/main` rather than from the checked-out branch, and only this log was staged by explicit path. Neither draft was staged, read, or altered.

**External state changed:** one Medium story published — title, body, preview image, five tags, and canonical as recorded above. No Ghost write, no send, no Buffer change, no Substack action, no schedule. Nothing was reshared to another platform.

**Open, in order:** (1) **the scheduler substrate** — this is now the second task in one day handed a dead slot by the launch catch-up burst, and unlike the Saturday note this one had no guard to refuse it; the Thursday Medium slot was simply missed and recovered by hand. (2) The task file's tag step describes a failure mode that does not match the real one and should be rewritten to the Down-then-Return method above; it lives outside this repository at `~/.claude/scheduled-tasks/gmg-thursday-medium/SKILL.md` and was not edited in this run. (3) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, and the next field-note draft commissioned `witness`.

## 2026-08-30 — Claude Code: Field Note 16 drafted `witness` against the standing commission, with its complete pack

**Client:** Claude Code (scheduled task `gmg-saturday-draft`, founder not present). **Branch:** `claude/field-note-16-draft-2026-08-30`.

**Nothing was published, sent, or posted.** The draft, its platform pack, and its artwork are built for founder review and are marked not approved and not authorized in their own frontmatter.

**This run is not the run that stood down.** `gmg-saturday-note` returned exit 20 nine hours late against a 09:30 posting slot and refused to post (entry above, same date). That guard governs a slot; this task drafts and holds no slot, changes no external state, and so was correct to proceed on the same late fire. The two are separate tasks and the distinction is recorded here because both fired in the same catch-up burst and a later reader would otherwise read one stand-down as covering both.

**Backfill was checked first and found clean.** Every note under `content/field-notes/` — fourteen, numbered 02 through 15 — already carries its platform pack, render script, carousel, feature image, review sheet, and gate entries. No approved essay is unshippable, so the run went to new work.

**The draft is `witness`, and the commission is cleared.** `**Standing commission:**` in `operating-cadence.md` read `witness`; this change set sets it back to `none`, per the rule that the task writing the note clears it in the same change set. The subject test passes rather than being forced: a bearing wall comes out of a man's house on a schedule he did not set, and there is no move available to him inside the event. The three gates hold — no imperative aimed at the reader and no closing question, nothing liftable as "so a man should ___", and the cared-for man is the grammatical object throughout while the givers' verbs carry the piece. The only second person in the essay sits inside a quoted phrase, which is the construction both approved witness pieces already use.

**Its shape is deliberately opposite to Wednesday's.** Field Note 15 was interior and analytical — a machinist's argument about tolerance run as self-audit, `assignment`. This one is external and event-driven: temporary shoring, a funeral, and nine months of other people's Saturdays. The counter-case is delivered through the mechanism rather than as a standalone disclaimer, because a third repetition of "one driveway is not a statistic" would be a formula rather than an honesty.

**Three photographs were generated and one was rejected.** Continued the most recent *Editorial Photography Request* thread in the founder's pinned **Grown Men Grow** project. The floor image was rejected on the first attempt: the putty knife had no handle at all, the blade running straight into the fingers with the thumb flat on the blade face. Regenerated with the tool's construction described explicitly — blade seated into a wooden handle with a visible ferrule, hand on the handle only — and accepted on the second. The exact prompts, the rejection, and the source paths are recorded in the draft's production notes.

**Two composition defects were found in review and fixed before the run ended.** Slide 7's centre crop of a tall source kept only three of the four filled holes while the copy names four; the box was made narrower and shorter so the whole run shows and clears the headline it had begun to overlap. Slide 4's quoted offer rendered without quotation marks, so *Let me know if you need anything is a wall built with no path to the ground* read as the publication addressing the reader rather than as a quotation; quote marks were added to the slide and to the note's carousel source together.

**Files changed:** `drafts/a-temp-wall-comes-out-in-april.md` and `drafts/field-note-16-platforms.md` (new, unapproved); `scripts/render-field-note-16.mjs` (new); `scripts/verify-repository.mjs` (render script in `requiredFiles`, a `validateAssetFamily` line for the carousel, the feature-image family 17 → 18, three entries in `editorialSources`, and the review sheet); `scripts/render-review-contact-sheets.mjs` (one sheet entry); `docs/technical/operating-cadence.md` (commission cleared); this log; and the generated assets — three source photographs, seven carousel pairs, the feature image pair, and the review sheet.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus fatal GScan on the packaged zip, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail); `node scripts/verify-repository.mjs` (540 tracked files, 45 JavaScript files, 20 review sheets, 55 editorial source images); `bash scripts/verify-svg-xml.sh` (166 SVG files); `git diff --cached --check` clean; staged file list reviewed by explicit path.

**The checkout is shared and was treated as such.** A concurrent session moved `HEAD` from `3e4c0a2` to `99681e3` during this run, landing #142 and #143. This branch sits on `99681e3`, every path was staged explicitly rather than with a broad `git add`, and no file belonging to another session was staged or altered.

**External state changed:** none in the publication. Three images were generated in the founder's ChatGPT project and downloaded. No Ghost write, no send, no post, no Buffer change, no schedule.

**Open, in order:** (1) **Field Note 16 needs a founder decision** — essay, pack, and artwork are delivered together and none of it is approved; it holds no slot in `publication-order.md` and should not get one before approval. (2) The scheduler substrate remains the live defect, now cited by three consecutive entries: tasks fire in a catch-up burst on application launch rather than at their slots, and it cannot be fixed from inside this repository. (3) Whether this week's Substack Note 2 goes out late by hand or is skipped, still a founder call from the entry above. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, and the A/B suspended until thirty recipients. The stance commission is no longer open — it was consumed by this run.

## 2026-08-30 — Claude Code: Field Note 16 approved and promoted into `content/`

**Client:** Claude Code (interactive, founder present and approving). **Branch:** `claude/approve-field-note-16-2026-08-30`. Follows the drafting entry above from the same date.

**The founder approved Field Note 16 in one pass** — essay, platform pack, and artwork together. `a-temp-wall-comes-out-in-april.md` moved from `drafts/` to `content/field-notes/` and its pack to `content/distribution/field-note-16-platforms.md`, both by `git mv` so no stale copy remains under `drafts/`. Frontmatter on both now reads `status: founder-approved` with the approval date, and `publication_authorized` stays `false` on both: approval covers the work and a register position, not a Ghost slot.

**Nothing was published, sent, scheduled, or posted.** No Ghost write, no newsletter binding, no Buffer change, no social post.

**The payload was proved to build rather than assumed to.** `node scripts/stage-next-field-note.mjs a-temp-wall-comes-out-in-april 2026-11-24T13:00:00Z --dry-run` resolved the source and the feature image, passed the `status: founder-approved` refusal, emitted 6,254 characters of HTML containing only `<p>`, `<h2>`, and `<em>`, and printed the alt text rather than `MISSING`. This matters because the unit-test count did not change when the note moved into `content/`, so a passing suite alone would not have shown that anything new was exercised. The dry run does not touch the network and does not exercise any of the five Ghost calls.

**The register cost is real and is not absorbed silently.** The note opens on framing a temporary wall, a build-trade opening on the same reading that counted anchor bolts as one. Appended at 15 it puts positions 13, 14, and 15 at gutters, machining, and framing — a run of three, where the 2026-08-26 reorder had brought the longest run down to two — and it places the third `witness` piece two slots after the second, against the preference that they run four apart.

**That is the fourth append in a row to record the same strain, so the fix was worked out instead of deferred again.** `publication-order.md` now carries a specific one-move recommendation — move `anger-is-a-terrible-manager` from 4 to 14, the only unconstrained non-build-trade piece in the front half — with the three binding constraints checked against it rather than asserted. **It is not applied.** Approving a note is not approving a reorder of nine other slots, and that register's own rule makes order a founder decision.

**The stance commission was already cleared** to `none` in the drafting change set, per the rule that the task writing the note clears it. It is not re-set here; commissioning another `witness` is a founder call. The corpus now reads three `witness` pieces in sixteen. The corpus test is not due — the marker reads a bank of 15 with the next measurement at 21, and the checker reads that threshold from the marker.

**Files changed:** `content/field-notes/a-temp-wall-comes-out-in-april.md` and `content/distribution/field-note-16-platforms.md` (moved from `drafts/`, frontmatter promoted); `docs/technical/publication-order.md` (position 15, plus the recommendation section); `docs/technical/decision-log.md` (approval entry); this log. No script, gate, or asset change — the gate entries and artwork landed in the drafting change set and needed nothing further.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus fatal GScan on the packaged zip, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail); `node scripts/verify-repository.mjs` (562 tracked files, 45 JavaScript files, 20 review sheets, 55 editorial source images); `bash scripts/verify-svg-xml.sh` (166 SVG files); `node scripts/stage-next-field-note.mjs … --dry-run` as recorded above; `git diff --cached --check` clean; staged file list reviewed by explicit path.

**External state changed:** none.

**Open, in order:** (1) **the register reorder recommended above** — a founder call, not urgent, and the arithmetic is done so it can be taken or refused in one step. (2) Whether to commission another `witness`; the bank is three in sixteen against a stated preference of roughly a quarter. (3) The scheduler substrate, now cited by four consecutive entries: tasks fire in a catch-up burst on application launch rather than at their slots, and it cannot be fixed from inside this repository. (4) Whether this week's Substack Note 2 goes out late by hand or is skipped. (5) Carried forward unchanged: the distribution problem, the newsletter list at one member, and the A/B suspended until thirty recipients.

## 2026-08-30 — Claude Code: the four open items closed

**Client:** Claude Code (interactive, founder present). **Branch:** `claude/close-open-items-2026-08-30`. Third entry of this date, following the drafting and approval entries above. Founder instruction: finish the work and stop carrying open items.

**Nothing was published, sent, or posted.** One deliberate non-action is recorded below.

**1. The register reorder is applied.** `anger-is-a-terrible-manager` moved from 4 to 14, shifting positions 5 through 14 up one. Build-trade openings now sit at 6, 8, 10, 12, 13, 15 — longest run two, the best this bank allows — and the three `witness` pieces at 8, 12, 15. Positions 1 and 2 are staged on Ghost and did not move. **The constraint checker was proved rather than trusted:** the order was deliberately broken by moving `nobody-rigs-to-the-breaking-strength` between `rest-is-not-a-reward` and `you-cant-outwork-a-wrong-direction`, `verify-repository.mjs` failed by name on the immediacy constraint, and the correct order was restored and re-verified. The cost is real and recorded: `anger-is-a-terrible-manager` moves out ten weeks.

**2. The stance commission is set to `witness`.** The bank reads three in sixteen against a stated preference of roughly a quarter. The commission instructs the next draft task what to look for; it does not licence forcing a subject that fails the three gates, and a task that cannot find one leaves it standing.

**3. The scheduler substrate is fixed at the layer that owns it.** This is the item three previous entries called unfixable from inside the repository, and that was correct — the fix is in the scheduled-task configuration on this machine. The cause was measured rather than guessed: every Monday-through-Thursday morning task fired within a minute of its slot, and every Friday and Saturday task fired hours late (Friday analytics +338 minutes, Saturday note +556, Saturday draft +481). Tasks fire while the desktop application is open and otherwise on next launch, so the application being closed on Friday and Saturday mornings is the whole of it.

Three crons moved into the window the machine has demonstrated: `gmg-saturday-note` 09:15 → 18:15, `gmg-saturday-draft` 10:36 → 18:45, `gmg-friday-analytics` 09:08 → 14:30. Next runs confirmed from the scheduler as Saturday 18:20 ET, Saturday 18:50 ET, and Friday 14:38 ET. Only the note has an editorial slot; the other two have none.

**The one editorial judgement made on the founder's behalf** is moving the Saturday Note slot from 9:30 AM to 6:30 PM in `publish-timing.md`, which is founder-approved research. The rationale in that row distinguishes weekend from weekday rather than morning from evening, so the move spends none of it, and it is reversible in one line. It rests on one observation that the application was open at 6:36 PM Saturday against two that it was not open at 9:30 AM — thin for the new time, strong against the old. Three things changed together because a half-move is worse than none: the row, the cron, and the `--slot 09:30` → `--slot 18:30` argument in the task's `SKILL.md`. The guard computes lateness against the slot it is passed, so moving the cron alone would have produced a task that fires on time and then refuses itself as nine hours late.

**Known limitation, stated rather than left to be discovered:** the scheduled-task configuration lives only on this machine and is not tracked in `ops`, so nothing asserts this fix is still in place. `publish-timing.md` now records the intended cron and slot values, which is the nearest thing to an assertion available without building one.

**4. This week's Substack Note 2 is skipped, and was not posted.** The lateness guard declined this exact action nine hours late for a stated reason, and posting by hand now would override that guard rather than resolve it. `AGENTS.md` requires explicit founder authorization for that specific action, and an instruction to close open items is not that. The copy is unaltered in `content/distribution/field-note-04-platforms.md` and remains available if the founder wants it posted by hand. The next Note 2 goes out at the new evening slot on 2026-09-06.

**Files changed:** `docs/technical/publication-order.md` (reorder applied, recommendation section replaced with the applied record), `docs/technical/operating-cadence.md` (commission → `witness`), `docs/technical/publish-timing.md` (Saturday row and a section explaining the move), `docs/technical/decision-log.md`, and this log. Outside the repository: three scheduled-task crons and `~/.claude/scheduled-tasks/gmg-saturday-note/SKILL.md`.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus fatal GScan on the packaged zip, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail); `node scripts/verify-repository.mjs` (562 tracked files), plus one deliberate failure and recovery as described; `bash scripts/verify-svg-xml.sh` (166 SVG files); `git diff --cached --check` clean; staged file list reviewed by explicit path.
...
## 2026-08-31 — Claude Code: Week 4 staged; the register ran dry for the first time and the staging script ran for real

**Client:** Claude Code (scheduled task `gmg-monday-staging`, founder not present). **Branch:** `claude/monday-staging-2026-08-31`. The task fired at 09:31 EDT against a 09:30 slot — on time, which is the weekday-morning pattern `publish-timing.md` records.

**The hold was checked first and does not fire.** `**Active hold:**` reads `none`. No death, illness, or crisis is known to this run. The day's news carries US strikes on Iranian launchers at Hormuz, a Venezuelan oil announcement, Hurricane Karina in the Pacific, and flooding in Asia; none of it makes an essay about calling a friend on an ordinary Tuesday read as oblivious, which is the test the hold section sets rather than whether the essay mentions the news. The queued copy names no one, asserts no event, and carries no factual claim that could be wrong — it is in the generic present throughout. No escalation exists in the replies to be unresolved: the list stands at one member. Ghost and Buffer both answered their own pinned credentials with the expected account and organization.

**This is the first week the register ran dry, which means the staging script's network half ran for the first time in its life.** `publication-order.md` predicted exactly this: every previous Monday found a slot already scheduled, so the task verified rather than staged and the code below the argument check had never executed. Ghost held three posts, all published, and nothing scheduled. Position 3 — `friendship-has-a-maintenance-schedule` — was the lowest-numbered note with no Ghost post, and the three binding constraints put it before `rest-is-not-a-reward`, which is unstaged, so nothing about the order was strained by taking it.

**Staged and verified independently of the script's own report.** `node scripts/stage-next-field-note.mjs friendship-has-a-maintenance-schedule 2026-09-01T12:00:00Z` after the mandated dry run. The post is `scheduled` for 2026-09-01T12:00:00Z — 8:00 AM ET, EDT being UTC-4 — with the newsletter bound to `default-newsletter` and `email_segment: all`, feature image set, and alt text present. A second read with `formats: "html"` confirmed a 5,163-character body, five `<h2>` and twenty-seven `<p>`, opening and closing on the approved sentences. That read was not ceremony; see the defect below.

**The verify block lies about the body, and this run is what surfaced it.** Its last line printed `html length : 0` on a correctly staged post. The read-back at `scripts/stage-next-field-note.mjs` passed `include: "newsletter"` and no `formats`, and Ghost defaults a post read to mobiledoc — so `post.html` was undefined and the line printed `0`. It would have printed `0` for a genuinely empty post as well, so the check could not distinguish the two states it exists to tell apart, which is the silent-pass class `AGENTS.md` forbids outright. Fixed here: the read requests `formats: "html"`, and a zero-length body now says so in words rather than as a bare number. Nobody could have seen this before today, because no real run had ever reached that line.

**The metadata was diffed against the repository source and agrees.** `buildPostPayload` derives `title`, `custom_excerpt`, `meta_title`, `meta_description`, and `email_subject` for this note; all five match what Ghost holds. This note carries no `# Metadata` section, so `meta_title` is the derived `Friendship Has a Maintenance Schedule | Grown Men Grow`. **That is the string Medium's URL importer will take as Thursday's headline**, suffix included — the same behaviour the 2026-08-30 Medium entry recorded and corrected by hand. It is not a divergence to fix here; it is a standing consequence of the derivation, and Thursday's task should expect it again.

**The feed-tile check passes both ways.** `verify-repository.mjs`'s photograph-exclusivity gate is green across the corpus, and the live comparison the gate cannot make was run: the most recent Instagram post Buffer has actually **sent** is the Field Note 4 carousel of 2026-08-27, whose tile is built on `hallway-duffel-set-down.png`. This week's tile is `garage-doorway-call.png`. Different photograph, different composition, no grid collision.

**Buffer's queue was empty and now holds four scheduled posts**, mode `customScheduled`, scheduling `automatic`, every one verified by reading the queue back rather than by trusting the mutation's reply. Bluesky fragment Tue 16:00Z; LinkedIn Wed 14:00Z; Instagram carousel Thu 13:00Z with seven slides and seven alt-text strings attached; Bluesky canonical link Sat 22:30Z. The seven carousel PNGs were uploaded to Ghost storage as `fn3-c1.png` through `fn3-c7.png` for public URLs, following the naming the earlier weeks set.

**Every string was read out of the approved files by script rather than retyped**, so "verbatim" is a property of how the copy got there and not a claim about care. The one substitution is `[canonical link]` → the live URL, which is the pack's own placeholder convention and is visible in every previously sent post.

**The Saturday fragment went out at the schedule of record's hour, not this task's.** `publish-timing.md` moved the Saturday row from 9:30 AM to 6:30 PM on 2026-08-30; this task's own instruction file still said 9:30 AM. The task file defers to `publish-timing.md` in its own words, so the queue follows the register, and the stale hours in `~/.claude/scheduled-tasks/gmg-monday-staging/SKILL.md` were corrected in both places they appeared so the next run does not have to re-derive it. Worth naming as a pattern rather than an incident: this is the second task file in three days found carrying instructions its own repository had already superseded, after the Medium tag procedure on 2026-08-30. Task files live outside this repository and no gate can see them.

**The Week 4 phone kit is built** at `Week 04 — Friendship Has a Maintenance Schedule`, in the founder-ruled naming. Three action folders — Substack Note 1 (Tue), Substack Note 2 (Sat), the optional Thursday story link sticker — plus the no-action backup folder holding all four Buffer copies and the seven slides with their alt text. The `READ ME` leads with the changed Saturday hour, because a founder following last week's habit would post an hour of the day that no longer has a slot.

**No new reader-facing prose was written this run.** Everything that reaches a reader is verbatim approved copy. The kit's `READ ME` is operating instructions addressed to the founder, not publication copy, so the nine tests in `editorial-underpinning.md` do not apply to it; they were not run against it and are not claimed to have been.

**Files changed:** `scripts/stage-next-field-note.mjs` (verify read-back), `docs/technical/publication-order.md` (positions 1 and 2 marked published, position 3 scheduled), and this log. Outside the repository: this task's own `SKILL.md`, and the iCloud kit.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs` (17 files); `pnpm --dir theme install --frozen-lockfile`, exit 0; `pnpm --dir theme test`, no fatal issues on Ghost 6.x; `pnpm --dir theme zip` plus fatal GScan on the packaged zip, no fatal issues; `node --test 'scripts/test/**/*.test.mjs'` (128 pass, 0 fail); `node scripts/verify-repository.mjs` (562 tracked files, 45 JavaScript files); `bash scripts/verify-svg-xml.sh` (166 SVG files); `git diff --check` clean; staged file list reviewed by explicit path.

**The checkout is shared and was treated as such.** `origin/main` moved from `54fdc8b` to `5dd7769` during this run, landing #147. This branch was cut from `origin/main` after that fetch rather than from the stale local `HEAD`, and every path was staged explicitly.

**External state changed:** one Ghost post created and scheduled with the newsletter bound; eight images uploaded to Ghost storage (one feature image, seven carousel slides); four Buffer posts created as scheduled. Nothing published, nothing sent, nothing posted to a social surface. The Ghost post sends on Tuesday at 8:00 AM ET under the standing authorization for approved copy at schedule-of-record slots.

**Open, in order:** (1) **Thursday's Medium import will again import the title with the ` | Grown Men Grow` suffix** — expected, recorded above, and a hand fix in that task rather than a defect here. (2) Whether the Saturday 6:30 PM slot is actually reachable is still an open question with one observation behind it; if the Substack note misses again this week, `publish-timing.md` says the conclusion is that no unattended Saturday slot works on this machine. (3) Task instruction files drift out of step with the repository and nothing detects it; two instances in three days. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, and the A/B suspended until thirty recipients. The standing commission reads `witness` and belongs to the next draft task, not to this one.

## 2026-09-01 — Claude Code: the Tuesday note stood down 378 minutes late, and the weekday premise it exposed

**Client:** Claude Code (scheduled task `gmg-tuesday-note`, founder not present). **Branch:** `claude/tuesday-note-standdown-2026-09-01`. The task fired at about 18:14 EDT against an 11:45 cron and a 12:00 slot.

**Nothing was posted, and that was the correct outcome.** `node scripts/note-task-preflight.mjs --task gmg-tuesday-note --slot 12:00 --note 1` exited `20` — stand down — with `reason` reading `378 minutes past the 12:00 slot, beyond the 60-minute grace window`. Per the task's own exit-code table that ends the run: no composer was opened, no copy was staged, and no browser write of any kind was attempted. The preflight released its own lock; `.locks/` was verified empty afterwards rather than assumed, so a legitimate retry is not blocked.

**The essay itself published on time.** The preflight resolved the publication week to `friendship-has-a-maintenance-schedule`, published `2026-09-01T12:00:00Z` — 8:00 AM ET, the Ghost slot of record — from `content/distribution/field-note-03-platforms.md`. The Monday staging run set that up and it fired as scheduled. The failure is confined to the Tuesday midday fragment.

**The profile was read to establish what actually happened, not to post.** One read-only tab on `substack.com/@grownmengrow/notes`, closed immediately after. The page showed the publication identity with the `Edit profile` control present, and the top note dated `7d` — the 2026-08-25 note. **Note 1 for this essay is not live.** This mattered enough to spend the check: a stand-down verdict says only that this run declined to post, not that nothing posted, and the report would otherwise have been a claim about the task instead of about the world.

**The Saturday section's mechanism holds; its generalisation does not.** `publish-timing.md` recorded on 2026-08-30 that scheduled tasks fire while the desktop application is open and otherwise on next launch, measured across ten tasks — every Monday-through-Thursday task on time, every Friday and Saturday task hours late. Today's miss is a Tuesday, and the specific row that survey cites as on time is `Tuesday note 11:45`. So the mechanism is unchanged and correct, while "weekday slots are reachable" turns out to have been one week's application usage read as a standing property. Recorded in `publish-timing.md` under **The weekday premise broke on a Tuesday**.

**No slot was moved.** Changing the Tuesday row would be a second timing change made from a single miss — the same thinness the Saturday section flagged about its own evidence — and the Saturday move was founder-directed. The question of whether the Tuesday fragment should remain an unattended scheduled slot is left open below rather than answered here.

**No copy was written, edited, or improvised.** The approved Note 1 text was lifted by the preflight from the pack and is unchanged in the repository. Because no reader-facing prose was produced this run, the nine tests in `editorial-underpinning.md` do not apply and are not claimed to have been run.

**Files changed:** `docs/technical/publish-timing.md` (new observation section) and this log. No script, gate, or content file was touched.

**Verification, each gate named and run:** results recorded in the pull request; every `gate:` line in `AGENTS.md`'s declared block was executed by explicit path, plus `git diff --check` and a reviewed staged file list.

**External state changed:** none. One public Substack profile page was read and the tab closed. No post, no draft, no composer, no Ghost or Buffer mutation, no account setting.

**Open, in order:** (1) **Whether the Tuesday 12:00 PM fragment slot should stay unattended is now a founder decision** — it has the same failure mode as the Saturday note, and the Week 4 phone kit already carries this note's copy in its Substack Note 1 folder, so the founder can post it by hand if they still want it out. (2) This week's Note 1 did not go out at all; Friday's analytics task reads slot timestamps as its baseline and should expect a gap rather than a late post. (3) Task instruction files still drift out of step with the repository and nothing detects it — third instance in five days, and this one is not a drift but its mirror: `SKILL.md` and the schedule of record agree, and the machine did not do what either says. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, and the standing commission reading `witness` for the next draft task.

## 2026-09-02 — Field Note 17 draft, "The Rod Goes First", written against the standing `witness` commission

**Client:** Claude Code (desktop), the `gmg-wednesday-draft` scheduled task, fired 10:01 EDT against its slot.

**The Chrome extension was unreachable for the whole run, so there is no new photography.** `tabs_context_mcp` was called twice, before any other work and again before image work, and both returned not connected. Image generation happens only in the founder's saved ChatGPT project through Chrome, so no substitute path exists and none was attempted. This is stated first because the pipeline requires it stated first: the unit is delivered complete in every other respect and short exactly three photographs.

**Backfill first, and it was already clean.** Every one of the fifteen founder-approved notes was checked against the complete-unit table in `content-pipeline.md` — essay, platform pack, render script, carousel, feature image, review sheet, and both gate entries. All fifteen hold all seven. No backfill was owed, so the run proceeded to new work.

**The draft is `witness`, which is what the commission asked for and not a label pinned to an assignment piece.** The subject is a sacrificial anode: a rod that corrodes in the dark for years so the tank does not, and that is already spent by the time anything about it is visible. The subject test passes because there is no move available to the man inside the event. All three gates hold — no imperative aimed at the reader and no closing question, nothing liftable as "so a man should ___", and the cared-for man is the grammatical object throughout while the givers' verbs carry every human beat. `**Standing commission:**` in `operating-cadence.md` is **left at `witness` and not cleared**, because the marker's own rule is that a draft task clears it in the change set that *lands* the note, and nothing landed: this is an unapproved draft in `drafts/`, not a note in `content/`.

**The essay is never written in second person, and that is load-bearing.** It says "a water heater" and "the tank," never "your water heater." Second person would hand the reader the socket and the breaker bar and convert the piece into an assignment; whoever pulls the rod is not him.

**The counter-case got a fourth handling rather than a third repetition.** The two earliest `witness` pieces run it as a standalone disclaimer paragraph and Field Note 16 runs it as a section delivered through the mechanism. Here it is one flat sentence with no room of its own — plenty of tanks have nothing left in them, they run exactly like the good ones, and one morning one is on the floor. A third repetition of a structural move is a formula, which is the reasoning Field Note 16's own production notes recorded when it declined the second.

**The artwork is type-led and mark-led, which is the visual system's stated answer and not a shortcut.** `editorial-visual-system.md` provides for a type-led composition where an article lacks photography of its own, and `Marginalia` exists for exactly this. Seven slides, a feature image and a review sheet render and pass the gates today. The signature vocabulary is an **anode-section mark** — a rod hanging beside a tank wall line notched with three pits, drawn only as thick as the material left in it, full at the open, eaten opposite the pits through the middle, and reduced to a bare core wire on the close while the pits are drawn closed and filled. It duplicates none of the twelve vocabularies already in service. The renderer composes no photograph at all, so the cross-article assertion holds trivially; it is kept because the image-led rebuild will need it.

**Three things were fixed by looking at the render rather than by trusting it.** The pits read as specks at feed-thumbnail size and were enlarged with the wall line weighted to match; slide 7 hung the spent rod above its text with a dead band between, and was rebuilt with the last fact first and the bare wire beneath it; and the feature image was a single mark on open paper, which read as an unfinished page, and was rebuilt as a three-section specimen plate that uses the landscape width. The `feature_image_alt` was rewritten to describe the plate that now renders, because alt text that describes an earlier composition is worse than none.

**The three photography prompts are written and recorded, not deferred to a later reading of the note.** They sit in the draft's production notes in full, each shaped to the house prompt with its own `Avoid:` list, naming the exact `assets/source/editorial/` paths they land at. A later run with Chrome up generates, reviews at full resolution against the quality gate, drops them in, and rebuilds the image-led slides.

**The nine tests were run and the results are in the delivery.** Test 2 and test 7 are recorded with what came close rather than smoothed: the draft's fourth section was rewritten because its first version ended by excusing the mechanism to the reader, and test 7 names the man who is only ever the rod, who is answered inside the mechanism rather than consoled.

**Files changed:** `drafts/field-note-17-the-rod-goes-first.md` and `drafts/field-note-17-platforms.md` (both new, both marked not approved), `scripts/render-field-note-17.mjs` (new), `scripts/render-review-contact-sheets.mjs` and `scripts/verify-repository.mjs` (Field Note 17 entries — required file, carousel family, feature-image count 18 → 19, review sheet), and the rendered assets under `assets/drafts/instagram/field-note-17-carousel/`, `assets/drafts/ghost/feature-images/the-rod-goes-first.*` and `assets/drafts/review/field-note-17-carousel.png`. No file under `content/` was touched.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose` over the packaged zip; `node --test 'scripts/test/**/*.test.mjs'` (128 passing, 0 failing); `node scripts/verify-repository.mjs`; `bash scripts/verify-svg-xml.sh` (174 SVG files); plus `git diff --check`, `git diff --cached --check`, and a reviewed staged file list.

**External state changed:** none. Nothing was published, sent, posted, scheduled, or uploaded. No Ghost, Buffer, Medium, Substack, Instagram or ChatGPT mutation. Two failed local calls to the Chrome extension, which reached no site.

**Open, in order:** (1) **Three photographs are outstanding for Field Note 17** and the prompts are ready to run the moment Chrome is reachable; the image-led slides get rebuilt in the same change set. (2) The draft is unapproved and unscheduled and holds no slot in `publication-order.md`, which is correct for `drafts/` and becomes owed the day the founder approves it. (3) `**Standing commission:**` remains `witness` by design and clears only when a witness note lands in `content/`. (4) Carried forward unchanged: the Chrome extension's health is not asserted by any check, so a run that needs images finds out by failing; the distribution problem; the newsletter list at one member; the A/B suspended until thirty recipients; and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-02 — Field Note 17 photography generated; the type-led build replaced with the image-led one

**Client:** Claude Code (desktop), same session as the entry above, on the founder's instruction to retry Chrome and generate the three photographs.

**This corrects the entry above rather than replacing it.** That entry recorded three photographs outstanding and a type-led composition standing in for them. Both facts were true when written and both are now closed. The earlier entry stands unedited, per the append-only rule.

**Chrome came back on the third attempt.** The two failures recorded above were real; no configuration changed between them and this run, so the extension's reachability is not something this repository can predict or assert. That remains an open weakness: a run that needs images finds out by failing.

**All three images were generated in the founder's saved ChatGPT project, continuing the most recent *Editorial Photography Request* thread** rather than starting a new one, because the note's family is the established editorial style rather than a genuinely new one. Each prompt was sent verbatim as recorded in the draft's production notes, one at a time, waiting for each to finish. All three returned at 1024×1536, the 2:3 the house prompt asks for.

**One image was rejected at the gate and regenerated, and the rejection is recorded rather than dropped.** The first `water-heater-top-fittings` result put two arms in frame — one in a dark sweater sleeve wearing a watch, one bare well past the wrist — which reads either as two people in a utility closet or as one man whose sleeve vanished. That is an object-continuity tell, and `editorial-visual-system.md` is explicit that one obvious tell rejects the whole image. The lower hand's grip on the tank edge was also anatomically strained. It was regenerated with the frame restricted to a single arm and a matching addition to the avoid list. The replacement holds: one arm, one hand, correct hot, cold and anode-port layout with red and blue seals, thread tape at the fitting, no readable text or brand plate. It was not rescued with a crop, grain, or darkness, which the same section forbids.

**The weakest surviving element is named rather than smoothed.** In `tap-running-hot` the water column meets the basin with very little splash or standing water. It reads as a wet basin at editorial scale and it is not a synthetic tell, but it is the softest physics in the set, and it is recorded in both the draft's production notes and the source README so a later reviewer is not discovering it alone.

**A share dialog opened mid-run and was closed without using it.** An earlier click on the image's share control surfaced ChatGPT's share sheet, carrying X, LinkedIn and Reddit buttons. It was dismissed with Escape and no share target was touched. Nothing was posted, linked, or copied out. Recorded because the dialog appearing at all in an automated run is worth someone knowing.

**The composition moved from type-led to image-led and the note was rewritten to match.** Slides 1, 4 and 7 now carry the photographs and the strips slide was dropped, because the fittings photograph carries the no-signal beat that the two strips were saying twice. The run is image, type, mark, image, sparse, type, image, so no two consecutive slides share a skeleton. The feature image is now photograph-led with the two-state mark plate in the oxblood margin. Two crops were fixed after looking at the render: both sources put their subject low in a 2:3 frame, so a centred or top-biased slice cropped it out, and both were biased to the bottom band. The mark gained a `structure` colour because a dark tank wall line on an oxblood field made the pits meaningless.

**Every claim the note makes about its own artwork was rewritten to match what renders.** `artwork_status`, `feature_image_alt`, three of the seven slide alt lines, the visual-direction photography block, and the platform pack's `image_rule` all described a type-led build with no photography and would otherwise have been confidently wrong. Alt text describing a composition that no longer exists is worse than none.

**Files changed:** three new sources under `assets/source/editorial/` with README entries carrying their prompts, dispositions and consumers; `scripts/render-field-note-17.mjs` rebuilt image-led; `scripts/verify-repository.mjs` extended with the three source images and their dimensions; `drafts/field-note-17-the-rod-goes-first.md` and `drafts/field-note-17-platforms.md` corrected; and the re-rendered carousel, feature image and review sheet. No file under `content/` was touched. The draft remains unapproved and unauthorized for publication.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose`; `node --test 'scripts/test/**/*.test.mjs'` (128 passing, 0 failing); `node scripts/verify-repository.mjs` (585 tracked files, 58 editorial source images); `bash scripts/verify-svg-xml.sh` (174 SVG files); plus `git diff --check`, `git diff --cached --check`, and a reviewed staged file list. The source-count gate caught the three new images before they were registered, which is the gate working.

**External state changed:** none in the publication. Four image generations in the founder's ChatGPT project, in an existing thread, and three files downloaded to `~/Downloads` and copied into the repository. Nothing published, sent, posted, scheduled, or shared.

**Open, in order:** (1) The Chrome extension's reachability is asserted by nothing, and two of three attempts failed today. (2) `assets/source/editorial/README.md` documents sources through Field Note 13 and then stops; Field Notes 14, 15 and 16 never got entries, and Field Note 17's were added here. That gap predates this change set and is left named rather than fixed. (3) `**Standing commission:**` remains `witness` by design and clears only when a witness note lands in `content/`. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-02 — Field Note 17 approved and moved into content; a corrupted file caught at the boundary

**Client:** Claude Code (desktop), same session, on the founder's approval of the draft.

**A file this session committed was corrupted, and the corruption reached a pull request before it was caught.** `drafts/field-note-17-the-rod-goes-first.md` should be 178 lines; the version committed in the photography change set was **21,487 lines**, every original character interleaved with a copy of one prompt line — the signature of a string replace whose search term evaluated to empty. The cause was a slice-based replacement in the edit script that produced an empty search string; the mechanism is understood and the remedy below is structural rather than a promise to be careful.

**It was visible in the commit output and read past.** That commit reported `18 files changed, 21576 insertions(+)` for what was three PNGs and a handful of prose edits. The number was in front of me and I did not question it. That is the actual failure — not the bad replace, which is an ordinary scripting error, but treating a wildly wrong insertion count as noise.

**Every gate passed on the corrupted file, and they were right to.** `verify-repository.mjs` scopes its alt-text, corpus and cross-reference checks to `content/`, because a draft is deliberately outside the approved population. The file was in `drafts/`. So the corruption was undetectable by design until the moment of promotion — which is exactly when it surfaced, on the first read of the file after `git mv`. The design worked; it just means **a draft can rot silently for as long as it stays a draft**, and nothing reports it.

**Blast radius, stated precisely.** PR #155 merged the clean 178-line draft before the corruption existed, so `main` was never wrong. The corrupted version lives only in the unmerged commit on `photography/field-note-17`, which is being replaced by this change set. Nothing was published, sent, or staged from it. The three photographs, the render script, and the rendered assets in that commit are unaffected — the corruption touched one Markdown file.

**The note was rebuilt from the clean copy in `main` rather than repaired in place.** Every edit is re-applied through a helper that asserts each search string is non-empty and matches exactly once, plus a final line-count sanity bound. The first run of that rebuild failed its own assertion — `featalt: found 0 matches` — because the clean copy already carried a later revision of that line, which is the guard doing its job on the first attempt rather than on review.

**The approval itself.** `the-rod-goes-first` moves to `content/field-notes/` and its pack to `content/distribution/`, both with founder-approved frontmatter dated 2026-09-02. `publication_authorized` stays `false`: approval covers the work and a register position, never a Ghost slot.

**The register was reordered rather than appended, and the reasoning is in `publication-order.md`.** Appending at 16 would have put `a-temp-wall-comes-out-in-april` and `the-rod-goes-first` adjacent as witness pieces — worse than the gap-of-two that triggered the 2026-08-30 reorder. No insert-and-shift avoids a gap of two somewhere, so positions 7 through 16 were rearranged: build-trade longest run two, witness at 7, 10, 13, 16 with gaps of three. Ten slots move and the cost is stated there, including that the first witness piece now sits marginally earlier than the back-half preference wants.

**Verified by breaking it, not by a green run.** The new note's register row was deleted and the checker failed by name on the missing slot; the rest/direction adjacency was inverted and it failed by name on that. Both restored and re-verified.

**The standing commission is cleared.** `**Standing commission:**` reads `none`. The bank is 17 pieces with `witness` at four, or 23.5 percent, against the Stance section's "roughly a quarter" — the gap the two commissions existed to close. `**Corpus test last measured:**` is left at its 2026-08-26 value with the next due at 21; the bank is 17 and no measurement was made, so advancing the marker would record one that did not happen.

**A dependency fix rode along because it blocked the merge.** OSV reported two medium advisories against `qs` 6.15.3 in `theme/pnpm-lock.yaml`, unrelated to this work. `qs` is transitive under the single `gscan` devDependency through `express` and `body-parser`, both of which already declare ranges accepting 6.16.0, so a lockfile refresh fixes it with no override and no suppression. `pnpm --dir theme install --frozen-lockfile` and both GScan runs pass on the new lockfile.

**Files changed:** `drafts/field-note-17-the-rod-goes-first.md` → `content/field-notes/the-rod-goes-first.md` (rebuilt clean, approved frontmatter); `drafts/field-note-17-platforms.md` → `content/distribution/field-note-17-platforms.md`; `docs/technical/publication-order.md` (reorder plus rationale); `docs/technical/operating-cadence.md` (commission cleared); `docs/technical/decision-log.md`; `theme/pnpm-lock.yaml`; and this log.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs`; `pnpm --dir theme install --frozen-lockfile`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose`; `node --test 'scripts/test/**/*.test.mjs'`; `node scripts/verify-repository.mjs`; `bash scripts/verify-svg-xml.sh`; plus `git diff --check`, `git diff --cached --check`, and a reviewed staged file list — this time reading the insertion counts rather than skimming them.

**External state changed:** none. Nothing published, sent, posted, scheduled, or uploaded.

**Open, in order:** (1) **A draft cannot be checked by anything**, which is by design and which this session demonstrates has a cost — a corrupted draft is invisible until promotion. Whether `drafts/` should get a cheap structural check (frontmatter parses, line count plausible) is a founder call, not something to add unasked. (2) `the-rod-goes-first` holds position 16 and is unstaged; the Monday task will not reach it for months. (3) `assets/source/editorial/README.md` still documents sources through Field Note 13 and then skips to 17; Field Notes 14, 15 and 16 have no entries. (4) The Chrome extension's reachability is asserted by nothing. (5) Carried forward: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-03 — Thursday Medium import: "Friendship Has a Maintenance Schedule" published, after a destructive edit that cost one draft

**Client:** Claude Code (desktop), the `gmg-thursday-medium` scheduled task. The task fired against its 6:45 AM slot but the run was resumed and completed in the afternoon, so the import landed roughly eight hours after the 7:00 AM slot.

**The lateness is recorded and it changed nothing.** This task carries no preflight and no grace window, unlike the note tasks. The only timing rule it has is canonical-first, and lateness moves in the safe direction: the Ghost post was 55 hours old at import rather than the 46 it would have been at 7:00 AM, against a 40-hour minimum. No stand-down was owed and none was taken.

**Both preconditions were checked before anything was written.** `latestPublishedPost()` returned `friendship-has-a-maintenance-schedule`, published `2026-09-01T12:00:00Z`, status `published`. `https://grownmengrow.com/friendship-has-a-maintenance-schedule/` returned 200. Drafts were empty and the profile carried three published stories — "A Confession Can Still Be Selfish", "Call Your Friends Before There's a Reason", "Strength Has to Grow Up" — none of them this week's, so there was no double-import risk. Both surfaces were checked, because the profile feed lists published stories only and would not show a stale import draft.

**I deleted the body of the first imported draft, and the recovery is the reason this entry is long.** The importer's `Grown Men Grow 3 min read` line was selected with `Home` then `shift+End`. In Medium's editor `shift+End` selects to the end of the *document*, not the end of the line, and the following Backspace took all 43 blocks with it — 5,099 characters down to 112. Six `cmd+z` presses did not restore it; they unwound my own edits further, to an empty 2-block document. The draft was deleted and the URL re-imported from the canonical.

**Nothing reached a reader.** The damage was confined to an unpublished draft that existed for about four minutes. The published count read 3 before the incident and 3 after it, and the story that went out was built from a clean second import, verified block by block. The deleted draft was empty at the point of deletion — 0 words — and was my own artifact from minutes earlier, not founder material; leaving it would have planted exactly the stale import draft that step 2 of this task exists to catch.

**The second pass verified rather than assumed.** The clean import was recorded first at 43 blocks and 5,099 characters, and every edit was checked against that baseline: the title edit left the count at 43 and removed exactly 17 characters, the width of ` | Grown Men Grow`. The `min read` line was then taken with a triple-click, which selected 27 characters bounded to that paragraph and nothing else, and the count fell 43 → 42 with the closing `Originally published at` line still in place. The generalisation worth keeping is that `shift+End` is not a line selection in this editor and a paragraph-scoped selection must be confirmed before a destructive key follows it.

**The importer got the same three things wrong it always does, and a fourth that is new.** The title imported as the SEO `meta_title`, `Friendship Has a Maintenance Schedule | Grown Men Grow`, and was corrected to the post's `title` field. The `Grown Men Grow 3 min read` line was removed. The preview image imported empty and was set from the story's own feature image. The new one: **Medium's tag autocomplete silently substituted "Mental Health" for "Men"** — pressing Return commits the first suggestion, not the typed string, and "Men" sits third behind "Mental Health" and "Mental Health Awareness". It was caught on a read-back of the committed chips rather than assumed from the keystrokes. Clicking the correct option, by coordinate and then by element reference, did not commit either; three arrow-downs to highlight `Men (1.4K)` and then Return did. The five tags now read Friendship, Masculinity, Relationships, Personal Growth, Men — the pack's set, in a different order.

**The canonical was confirmed twice, before and after publication.** Before: Advanced Settings had "This story was originally published elsewhere" checked, and the field's `value` — not its placeholder, which reads `Type the canonical URL...` — was `https://grownmengrow.com/friendship-has-a-maintenance-schedule/`. After: a fetch issued inside the page returned HTTP 200 and 195,573 bytes of server-delivered source carrying `<link rel="canonical" href="https://grownmengrow.com/friendship-has-a-maintenance-schedule/"/>`. The check was run in the browser rather than with `curl`, which Medium answers 403.

**No name and no reshare.** The story publishes under the Grown Men Grow profile with no attribution beyond it. The post-publish dialog offered Facebook, LinkedIn and X; none was touched and it was dismissed. Other platforms receive reviewed native copy, never a Medium reshare.

**No copy was written or edited.** The essay, subtitle and tags are the founder-approved strings from `content/distribution/field-note-03-platforms.md` and the Ghost post. Because no new reader-facing prose was produced, the nine tests in `editorial-underpinning.md` do not apply and are not claimed to have been run.

**Files changed:** this log only. No script, gate, content or asset file was touched.

**Verification, each gate named and run:** results recorded in the pull request; every `gate:` line in `AGENTS.md`'s declared block was executed by explicit path, plus `git diff --check` and a reviewed staged file list.

**External state changed:** one Medium story published at `https://grownmengrow.medium.com/friendship-has-a-maintenance-schedule-e867bf464b9d`, and one empty Medium draft created and deleted in the course of the failure above. Nothing else. No Ghost, Buffer, Substack, Instagram or account-setting mutation. The drafts list is empty and the published count reads 4.

**Open, in order:** (1) **Medium's tag autocomplete will substitute a wrong topic for any short tag whose prefix matches a larger one**, and it is silent about it — "Men" is the case in hand, and the read-back of committed chips is what caught it. The task file tells a future run to enter tags one per call; it does not tell it to verify what landed. (2) The `shift+End` behaviour above is worth the same treatment, since the task file names the three known importer defects and this failure mode is not among them. (3) Carried forward unchanged: `the-rod-goes-first` holds position 16 unstaged; `assets/source/editorial/README.md` still skips Field Notes 14 through 16; the Chrome extension's reachability is asserted by nothing, though it was reachable on first contact this run; the distribution problem; the newsletter list at one member; the A/B suspended until thirty recipients; and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-05 — Weekly analytics readout: every slot fired, every surface is silent, nothing to moderate

**Client:** Claude Code (desktop), the `gmg-friday-analytics` scheduled task. **It ran on Saturday.** The task completed at 15:42 EDT on 2026-09-05; its slot was Friday 2026-09-04. This is the same catch-up behaviour `publish-timing.md` already records — a slot is real only if the desktop application happens to be open at it — and it is logged here as a third instance rather than treated as new. Lateness cost nothing: this task only reads.

**Ghost, the essay and its email.** `friendship-has-a-maintenance-schedule` published 2026-09-01T12:00Z, which is Tuesday 8:00 AM ET, exactly the schedule of record. Email: 1 recipient, 1 delivered, 1 opened, 0 failed. Click counts are not exposed on the post's email object and `/stats/` and `/links/` still answer 403 to an Admin API key, so 48-hour pageviews and per-post conversions remain uncollectable programmatically — unchanged, and already recorded as an open choice for whoever restarts the A/B.

**Member count is 1, and the one member is the founder's own address.** This is worth stating plainly because "1 member, 100% open rate" reads as a metric and is not one: the list has no external subscriber. The open event on 2026-09-01 was the founder opening his own newsletter. Week-over-week member delta: zero.

**The social slots all fired on time.** Bluesky Tuesday 12:00 ET, LinkedIn Wednesday 10:00 ET, Instagram Thursday 9:00 ET — each within six minutes of its slot. The Saturday 6:30 PM fragment is queued in Buffer for tonight and had not fired at the time of this readout. The Thursday Medium import landed roughly eight hours late and that is recorded in its own entry.

**Per-post engagement, this week against last.** Instagram 7 views / 2 reach (last week 10 / 2); LinkedIn 0 impressions / 0 reach (last week 2 / 2); Bluesky 0 likes, 0 reposts, 0 replies (unchanged, as every week). Reactions, comments, saves, shares and follows are 0 on every surface this week. Instagram's 28-day window (Aug 8 – Sep 4) reads 121 views, 20 reach, 20 interactions, 0 conversations started. Instagram followers: 2. Bluesky followers: 0.

**The trend is down, and the numbers are too small for that to mean anything.** Instagram views fell 10 → 7 and LinkedIn impressions 2 → 0. At this scale a three-view difference is one person scrolling, not a signal, and the protocol's own five-point noise floor exists for exactly this. No timing conclusion is drawn and none should be. The last human engagement on any surface was 2026-08-20.

**Moderation sweep: every surface checked, every surface empty.** Ghost comments 0 across all posts (Admin API, `comments/` total 0). Instagram comments — Business Suite inbox shows the explicit "No comments" empty state. Instagram direct messages — "No messages to show". Facebook comments and Messenger empty via the All messages view. Bluesky 0 replies and 0 quotes across all seven posts (public AppView API). LinkedIn comments 0 on all three posts (Buffer metrics). **Nothing in any escalation row of `community-moderation.md`. Nothing was replied to, hidden, removed, or reported.** Business Suite's own to-do list reports nothing outstanding.

**Qualitative pass: there is nothing to report, and that is the finding.** No comment, reply, direct message, quote, or reader disclosure exists on any surface. No question or objection has recurred, because none has been raised once. Principle 5 asks how the work lands; the honest answer this week is that it has not yet reached anyone who responded. The distribution problem already recorded is the whole of it, and no slot-tuning reaches it.

**Corpus balance check: skipped, correctly.** `**Corpus test last measured:**` reads 2026-08-26 at a bank of 15, next due at 21. The bank was derived from tracked files rather than taken from the log — 16 notes under `content/field-notes/` plus the launch essay — and is 17. **Four pieces to go.** The marker is untouched.

**No protocol decision is due.** The A/B remains suspended at step 3: `**A/B minimum recipients:**` is 30 and the list is 1. `**Standing commission:**` is `none`. So this readout appends here rather than to the `publish-timing.md` decision log, per that document's own rule.

**One measurement defect found in the tooling, worth the next run knowing.** Querying an unknown field on `Post.metrics` makes the Buffer API answer HTTP 200 with `"Not authorized to access this resource."` — an authorization message for what is a schema typo. The helper's `introspectType()` is what resolved it; the real field set is `name`, `type`, `unit`, `value`, `description`. A future run that reads that error as a dead credential will be wrong, and the credential probe passing alongside it is the tell.
...
## 2026-09-05 — Saturday Draft: Field Note 18 as a complete unit, and four platform packs that were never being checked

**Client:** Claude Code (desktop), the `gmg-saturday-draft` scheduled task, founder not present. **Branch:** `claude/field-note-18-draft-2026-09-05`. **PRs:** #161 (the unit) and a follow-up carrying this entry.

**Nothing was published, sent, posted, or scheduled.** The draft is not approved and holds no register position.

**The backfill audit ran first and came back clean, which is the reason there is a new draft at all.** Every one of the sixteen approved notes was checked against the complete-unit table in `content-pipeline.md`: essay source, platform pack, source photography, render script, seven-slide carousel, feature image, review sheet, and gate entries. Packs 02 through 17, render scripts 02 through 17, and carousels 02 through 17 are all present, every carousel holds seven slide pairs, and every note has a feature image. One correction to the pipeline document's own table, recorded rather than acted on: it says a note's feature image comes from `render-ghost-feature-images.mjs` and that the note gets added to that script's `assets` map. It does not. That script holds three entries — the launch essay, `start-here`, and `about` — and every field note's feature image is emitted by its own `render-field-note-NN.mjs`. The artifact exists for all sixteen; the row naming its source is stale.

**Field Note 18, "The Error Is Half the Difference", written to `drafts/`.** A spirit level can be seated out of true, and a level that is out does not read erratically — it reads by the same amount in the same direction every time, so every line struck off it agrees with every other line struck off it and the work checks out. Measuring twice cannot find it, because the second reading comes off the same vial and that agreement is the defect. The trade makes the tool argue with itself: set it down, mark the bubble, turn it end for end, read it again. Essay body is 1,321 words, inside the 1,100 to 1,400 band.

**Stance is `assignment`, and the subject test decided it rather than preference.** The reversal is a move the reader has and it costs thirty seconds, so a witness framing would have had to remove the only thing the essay is about. The standing commission in `operating-cadence.md` reads `none` — the Wednesday task cleared it when it landed the rod note — so the ordinary stance applies and this change set clears no commission and does not touch that line.

**It is the counterpart to Wednesday's note, not its sibling.** "The Rod Goes First" ran three days earlier as `witness`: an object spent in the dark protecting a tank that cannot know, no move available to anyone inside the event, closing on a fact. This one is external and in the reader's hands throughout and closes on what the test returns. Two witness pieces in one week would also have started reading as the labeled series the underpinning forbids.

**The punch list was the first subject considered and was dropped before anything was written.** Field Note 11 closes on "He is told to go finish the punch list on the other job," which is the most quoted line in that piece, and `punch-list-tailgate.png` is already claimed by it. An essay built on the punch-list mechanism would have collided with both the image and the line.

**Four platform packs were never being checked, and a fifth was about to join them.** The `packs` map in `verify-repository.mjs` was hand-curated, naming Essay 1 and Field Notes 2 through 13 and stopping there. Packs 14, 15, 16 and 17 have existed in `content/distribution` without ever being checked for their four required sections or for Medium tag legality. The population is now derived from tracked files and fails closed if it finds none. **Verified by breaking it in both directions rather than by observing a pass:** removing the Substack section from pack 18 fails with the named error; an illegal apostrophe tag on pack 17 — a pack the old list could not see — fails with the named error; both pass restored. This is the same curated-list defect the repository contract already describes elsewhere, found in a place the contract had not reached.

**Imagery: three photographs, one regenerated, all verified distinct before entering the repository.** Generated in the founder's saved ChatGPT project, continuing the most recent *Editorial Photography Request* thread rather than starting a new one, each reviewed at full resolution against the quality gate. All three are 1024x1536. The first `level-on-top-plate` result was rejected for composition — the level sat in a narrow band at the top of the frame with both ends cropped — and it carried no synthetic tell, so it failed on composition rather than believability; the only way to use it was the aggressive crop the visual system forbids as a rescue, so it was regenerated with an explicit framing constraint instead.

**The download step produced one image saved three times, and the check that caught it is worth keeping.** Each generated image is mounted several times in the ChatGPT page, so position in the document is not the identity it appears to be, and selecting by index returned three byte-identical files. Hashing every mounted image in place, grouping duplicates, and taking one member per group produced three distinct files, confirmed again on disk. A set of images that were silently the same photograph would have passed every visual review and then tripped the photograph-exclusivity gate for reasons nobody could see. The extension also blocks returning any URL carrying a query string, so signed image URLs cannot be read out; fetching in-page and hashing is the path that works.

**The signature mark shipped with a geometry defect on its first build and was fixed rather than accepted.** The bubble-vial mark is new to this note — a horizontal capsule with two graduation lines and a bubble off centre, its second state the same vial turned end for end with half the gap bracketed. At the first graduation spacing the bubble was wide enough to cover the line it sat nearest, so it rendered with one visible line and read as a dot in a capsule rather than as a reading off true.

**Concurrency, recorded because it nearly went wrong.** This session began on branch `claude/saturday-note-2026-09-05` with a neighbouring session's handoff entry staged in the shared index. No work was committed while that was true. The neighbour merged as #160 and its cleanup left the shared checkout on `main`, so a branch was created before anything was committed, and only explicit paths were staged throughout — never `git add -A`. Auto-merge on #161 was armed before this entry was written, which is the wrong order, and the race it invites then actually fired: CI went green while the entry was being written, #161 merged carrying only the first commit, and the handoff commit was left behind on a branch pointing at a merged pull request. The unit itself — draft, pack, render script, photographs, carousel, feature image, review sheet and gate entries — all landed; the record of it did not, which is the half that fails silently, because a merged PR and a clean local branch both look finished. It was recovered by branching from the merged `main`, cherry-picking the stranded commit, and opening a second pull request with auto-merge armed last. The standing lesson is unchanged and now has a second observation behind it: arm auto-merge as the final action, after the handoff entry is pushed, never before.

**Files changed:** `drafts/the-error-is-half-the-difference.md`; `content/distribution/field-note-18-platforms.md`; `scripts/render-field-note-18.mjs`; `scripts/verify-repository.mjs`; `scripts/render-review-contact-sheets.mjs`; three source photographs under `assets/source/editorial/`; seven carousel SVG and PNG pairs; the feature image pair; and the review sheet.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose`; `node --test 'scripts/test/**/*.test.mjs'` — 128 pass, 0 fail; `node scripts/verify-repository.mjs` — 608 tracked files, 22 review sheets, 61 editorial source images; `bash scripts/verify-svg-xml.sh` — 182 SVG files; plus `git diff --check` and a reviewed staged file list.

**External state changed:** none in the publication. One Chrome tab opened against the founder's signed-in ChatGPT session, four images generated in the saved project, three downloaded and moved into the repository, and the tab closed. Nothing published, sent, posted, scheduled, or shared.

**Open, in order:** (1) **Field Note 18 awaits founder review** — essay, pack, and artwork, approvable in one pass; it holds no register position and takes no slot until the founder gives it one. (2) The pipeline document's complete-unit table names `render-ghost-feature-images.mjs` as the source of a note's feature image, which is wrong for every field note; worth a correction in that document when it is next touched. (3) Carried forward unchanged: the Substack keystroke block and whether those slots become manual by declaration; the distribution problem; the newsletter list at one member; the A/B suspended until thirty recipients; `assets/source/editorial/README.md` skipping Field Notes 14 through 16 and now 17 and 18; `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`; and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-05 — Field Note 18 approved and moved into `content/`, register position 17

**Client:** Claude Code (desktop), same session as the Saturday draft run, founder present for the approval. **Branch:** `claude/approve-field-note-18-2026-09-05`.

**The founder approved Field Note 18 as delivered — "Approved." — and nothing was published, sent, posted, or scheduled.**

**Approval was read as covering the work, not a Ghost slot.** `AGENTS.md` gates publication, newsletter delivery, and posting behind explicit authorization for that action, and the founder named none. So the essay, the platform pack, and the artwork are founder-approved as written; `publication_authorized` stays `false` on both the essay and the pack.

**The register position was not a judgement call, and that is worth recording.** The earlier read was that a bare "Approved." matches the Field Note 14 precedent — approval of the work with no publication slot named — and that the note might sit approved but unregistered. The gate settles it mechanically: `verify-repository.mjs` fails any note in `content/field-notes/` that holds no slot in `publication-order.md`. Moving an approved note into `content/` and leaving it out of the register is not a state the repository allows, so approval of the work and a register position arrive together whether or not a slot is named.

**Appended at position 17, projected 2026-12-08.** Appending moves nothing: every scheduled slot and every projection above it is untouched, and none of the three binding cross-reference constraints involves it, because the essay refers to no other note. **The trade-opening adjacency is stated rather than hidden** — it opens on a spirit level directly after `the-rod-goes-first` opens on a water heater, which is a run of two. That is the tolerance `publication-order.md` already accepts as the best this bank allows; the 2026-08-26 reorder cut the longest run from five to two and stopped there. It was appended rather than slotted for separation because moving it up would displace projections to accommodate one opening, which that document forbids. Breaking the pair is a reorder decision and belongs to the founder; a section in `publication-order.md` now says so.

**No corpus measurement was made and the marker was not advanced.** `**Corpus test last measured:**` stands at 2026-08-26, a bank of 15, next due at 21. The bank is now 18. Witness sits at 4 of 18. The marker is left alone, because advancing it would record a measurement that was not made.

**The standing commission was not touched.** It reads `none`, which is correct: `assignment` is the ordinary stance and needs no commission, so this change set clears nothing.

**The approval couplings were proven to fire on this specific note rather than observed to pass.** Deleting its register row fails with "is approved but holds no slot"; deleting one alt-text line fails with "7 carousel slides but 6 alt text line(s)"; duplicating `closing_addresses_reader` fails with the corpus-test error. All three restored and passing.

**Files changed:** `content/field-notes/the-error-is-half-the-difference.md` (moved from `drafts/`, frontmatter updated to founder-approved); `content/distribution/field-note-18-platforms.md` (frontmatter updated, `publication_authorized` and `stance_note` added, the "until the founder approves" clause removed); `docs/technical/publication-order.md` (position 17 plus a section recording why it was appended); `drafts/README.md`. No script, gate, or asset file was touched, and no artwork was regenerated.

**Verification, each gate named and run:** `node scripts/verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose`; `node --test 'scripts/test/**/*.test.mjs'` — 128 pass, 0 fail; `node scripts/verify-repository.mjs` — 608 tracked files; `bash scripts/verify-svg-xml.sh` — 182 SVG files; plus `git diff --check` and a reviewed staged file list.

**External state changed:** none. Nothing published, sent, posted, scheduled, or shared, and no account setting touched.

**Open, in order:** (1) **Field Note 18 holds register position 17 and no Ghost slot** — publication, newsletter delivery, and posting each remain separately gated and none is authorized. (2) The pipeline document's complete-unit table still names `render-ghost-feature-images.mjs` as the source of a note's feature image, which is wrong for every field note; carried from the draft entry. (3) `assets/source/editorial/README.md` now skips Field Notes 14 through 18. (4) Carried forward unchanged: the Substack keystroke block and whether those slots become manual by declaration; the distribution problem; the newsletter list at one member; the A/B suspended until thirty recipients; `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`; and the open founder question about the unattended Tuesday fragment slot.

## 2026-09-07 — Claude Code: Week 5 staged; the register ran dry a second time and the staging script ran clean

**Client:** Claude Code (scheduled task `gmg-monday-staging`, founder not present). **Branch:** `claude/monday-staging-2026-09-07`. The task fired at 09:31 EDT against a 09:30 slot — on time, which remains the Monday-morning pattern.

**The hold was checked first and does not fire.** `**Active hold:**` reads `none`. No death, illness, or crisis is known to this run. The day's news carries an armed attack on a candidate for Ohio governor, a building collapse in Delhi, and a Kansas meatpacking labour agreement; the test the hold section sets is whether a reader who just saw the news would find the queued post tone-deaf arriving that morning, and an essay about scheduling rest rather than earning it does not touch any of it. The queued copy names no one, asserts no event, and makes no factual claim that could be wrong — it is in the generic present throughout. No escalation exists in the replies to be unresolved: the list stands at one member. Ghost and Buffer each answered their own pinned credentials with the expected account and organization.

**The register ran dry for the second time, so the staging script's network half ran for real again.** Ghost held four posts, all published, nothing scheduled. `publication-order.md` position 3 — `friendship-has-a-maintenance-schedule` — published on time at 2026-09-01T12:00:00Z, so position 4, `rest-is-not-a-reward`, was the lowest-numbered note with no Ghost post. The three binding constraints are satisfied by taking it: `friendship-has-a-maintenance-schedule` precedes it, and `you-cant-outwork-a-wrong-direction` at 5 sits immediately after it, which is the adjacency that constraint requires.

**The dry run was run first and reported no defects** — 3,127-character body, alt text present, `publish_at` ending in `Z`. Precisely what it proves is the payload; the five Ghost calls were still first run for real afterwards.

**Staged and verified independently of the script's own report.** `node scripts/stage-next-field-note.mjs rest-is-not-a-reward 2026-09-08T12:00:00Z` — 12:00Z is 8:00 AM ET, EDT being UTC-4. A separate read with `include: "newsletter,tags,authors"` and `formats: "html"` confirms `scheduled` for 2026-09-08T12:00:00.000Z, newsletter `default-newsletter`, `email_segment: all`, visibility public, feature image and alt text set, and a 3,213-character body with four `<h2>` and fourteen `<p>`. The verify block's `html length` line, fixed on 2026-08-31, printed a real number this time rather than the false zero that fix removed.

**The metadata was diffed against the repository source and all six fields agree.** `buildPostPayload` derives `title`, `custom_excerpt`, `meta_title`, `meta_description`, `email_subject`, and `feature_image_alt`; each matches what Ghost holds. This note carries no `# Metadata` section, so `meta_title` is the derived `Rest Is Not a Reward | Grown Men Grow`. **That is the string Medium's URL importer will take as Thursday's headline**, suffix included. It is not a divergence to fix here — it is the standing consequence of the derivation, recorded on 2026-08-31 and again now, and Thursday's task should expect it a third time.

**The feed-tile check passes both ways.** `verify-repository.mjs`'s photograph-exclusivity gate is green across the corpus, and the live comparison the gate cannot make was run: the most recent Instagram post Buffer has actually **sent** is the Field Note 3 carousel of 2026-09-03, whose tile is built on `garage-doorway-call.png`. This week's tile is `hammock-midday-rest.png`. Different photograph, different composition, no grid collision.

**Buffer's queue was empty and now holds four scheduled posts**, mode `customScheduled`, scheduling `automatic`, every one verified by reading the queue back rather than by trusting the mutation's reply. Bluesky fragment Tue 16:00Z; LinkedIn Wed 14:00Z; Instagram carousel Thu 13:00Z with seven slides and seven alt-text strings attached; Bluesky canonical link Sat 22:30Z, at the 6:30 PM slot that fired successfully for the first time on 2026-09-05. The seven carousel PNGs were uploaded to Ghost storage as `fn7-c1.png` through `fn7-c7.png`, following the naming the earlier weeks set.

**A read-path asymmetry in Buffer's asset schema, measured rather than recalled.** `createPost` takes alt text as `assets:[{image:{url, metadata:{altText}}}]`, but reading it back is `assets { ... on ImageAsset { image { altText } } }` — `ImageAsset` has no `metadata` field, and querying one answers HTTP 200 with a schema error. The introspected shape is `id image mimeType source thumbnail type`. Worth naming because the verification query is the one that catches a carousel queued without alt text, and a wrong field there fails in a way that looks like a credential problem.

**Every string was read out of the approved files by script rather than retyped**, so "verbatim" is a property of how the copy got there and not a claim about care. The one substitution is `[canonical link]` → the live URL, which is the pack's own placeholder convention and is visible in every previously sent post.

**The Week 5 phone kit is built** at `Week 05 — Rest Is Not a Reward`, in the founder-ruled naming. Three action folders — Substack Note 1 (Tue), Substack Note 2 (Sat), the optional Thursday story link sticker — plus the no-action backup folder holding all four Buffer copies and the seven slides with their alt text. The `READ ME` leads with the fact that nothing changed from last week, because the previous kit led with a changed hour and a founder skimming for the delta should find the answer rather than infer it.

**No new reader-facing prose was written this run.** Everything that reaches a reader is verbatim approved copy. The kit's `READ ME` is operating instructions addressed to the founder, not publication copy, so the nine tests in `editorial-underpinning.md` do not apply to it; they were not run against it and are not claimed to have been.

**Files changed:** `docs/technical/publication-order.md` (position 3 marked published, position 4 scheduled) and this log. No script or content file was touched. Outside the repository: the iCloud kit.

**Verification, each gate named and run:** results recorded in the pull request; every `gate:` line in `AGENTS.md`'s declared block was executed by explicit path, plus `git diff --check` and a reviewed staged file list.

**External state changed:** one Ghost post created and scheduled with the newsletter bound; eight images uploaded to Ghost storage (one feature image, seven carousel slides); four Buffer posts created as scheduled. Nothing published, nothing sent, nothing posted to a social surface. The Ghost post sends on Tuesday at 8:00 AM ET under the standing authorization for approved copy at schedule-of-record slots.

**Open, in order:** (1) **Both Substack notes are founder actions again this week** — the keystroke path into the composer is still refused by the auto-mode classifier, and the note tasks will degrade to a handoff at both slots. (2) Whether the Tuesday 12:00 PM fragment slot should stay unattended is still a founder decision, unchanged since 2026-09-01. (3) Thursday's Medium import will again take the ` | Grown Men Grow` suffix as the headline — expected, and a hand fix in that task. (4) Task instruction files still drift out of step with the repository and nothing detects it; nothing new was found this run. (5) Carried forward unchanged: the distribution problem, the newsletter list at one member, and the A/B suspended until thirty recipients. The standing commission reads `none`, so the next draft task is unconstrained on stance.

## 2026-09-08 — Claude Code: Substack Note 1 staged for Week 5; the keystroke path worked again

**Client:** Claude Code (scheduled task `gmg-tuesday-note`, founder not present). **Branch:** `claude/tuesday-note-2026-09-08`. The task fired at 11:45 EDT against its 11:45 slot — on time.

**The preflight decided, and it said wait.** `node scripts/note-task-preflight.mjs --task gmg-tuesday-note --slot 12:00 --note 1` returned exit `10`, fourteen minutes early, with `waitSeconds: 862`. The hold reads `none`. It confirmed this week's essay — `Rest Is Not a Reward`, published 2026-09-08T12:00:00Z, which is 8:00 AM ET — and lifted Note 1 out of `content/distribution/field-note-07-platforms.md`. The lock was taken and held across the wait, then released with its token after the handoff. Exit `10` is not exit `0`, and the distinction is the whole point of the code: the 2026-08-18 double post was two runs that each read the conditions as fine.

**The profile was verified twice, before and after the wait, because the first read is a claim about the past.** Both reads showed `Grown Men Grow` / `@grownmengrow` with an `Edit profile` control present — publication identity, session logged in — and both showed the top note dated Aug 25. This week's Note 1 was not already there. The Chrome extension was registered and local before either read.

**The keystroke path into the composer worked this week.** Monday's entry expected it to be refused and expected both note slots to degrade to a founder handoff; that expectation was wrong for this slot. The `type` action was attempted rather than assumed, per the task file, and it was not blocked. The record now stands at refused 2026-08-18, accepted 2026-08-25, accepted 2026-09-08, with no permission change in between — the block is intermittent and cannot be predicted, only tested.

**A `type` that reports success is not evidence that anything was typed.** The first attempt returned `Typed "Rest before the big thing..."` and the composer stayed empty: the click that should have focused the contenteditable was aimed with coordinates derived from an earlier 0.6-scale screenshot, and the modal had moved between the two. The keystrokes went to the page body, where the copy's spaces scrolled the page — which is also why the layout had shifted. Nothing was posted and nothing was drafted by that miss; it was confirmed harmless on a later read. The fix was to stop using coordinates: `find` resolved the contenteditable to a reference and the click went to the reference instead. **Screenshot coordinates go stale the moment the page reflows, and a modal that fades in reflows. Prefer an element reference for anything that must land in a specific field.**

**Staged verbatim and verified two ways.** The copy reads `Rest before the big thing, not just after. Nobody admires the man who shows up to the decisive week already spent.` — one paragraph, no link, no hashtags, no image, matching `field-note-07-platforms.md` line 47 exactly. The accessibility tree truncated its text node at `decisive week `, so the tail was confirmed by a zoom on the rendered text rather than by trusting the tree; both agree on everything they both cover. The Post button went from disabled to active, which is the composer's own signal that it holds content.

**Nothing was posted. The composer is open and waiting for the founder**, per the standing 2026-08-25 ruling that the agent stages and the founder clicks Post. The live profile was checked in a second tab and closed again — never by reloading the staged tab, which would discard the thing the founder is waiting to click — and the top note there is still Aug 25.

**Files changed:** this log only. No script, content, or distribution file was touched.
...
## 2026-09-09 — Claude Code: Field Note 19 drafted complete — "The Line Was Capped, Not Removed"

**Client:** Claude Code (scheduled `gmg-wednesday-draft`, founder not present). **Branch:** `claude/field-note-19-draft`, cut from `origin/main` at `fdb68d1`.

**Backfill was checked first and found nothing owed.** Every one of the seventeen approved notes already carries its pack, render script, carousel, feature image, review sheet, and both gate entries — packs and carousels run 02 through 18 with no holes, and all seventeen feature images are present. The audit was run against the complete-unit table in `content-pipeline.md` and derived from the directories rather than from a list, so the run went on to new work.

**The draft is complete as a unit and is not approved for anything.** Essay, platform pack, three new photographs, render script, seven-slide carousel, feature image, review sheet, and gate entries all landed in one change set. `drafts/field-note-19-the-line-was-capped-not-removed.md` carries `status: draft — NOT founder-approved` and `publication_authorized: false`; the pack sits beside it in `drafts/`, not in `content/distribution/`, because the boundary in `content-pipeline.md` puts unapproved work in `drafts/` and only the founder moves it across. That placement also keeps the note out of the approved-corpus checks, which is correct — it is not part of the bank until the founder says so.

**Subject and stance.** A branch of pipe that lost its fixture and was capped where it stood rather than cut back to the main — the trade's dead leg — read against the things a man stopped doing but never ended. `stance: assignment`, with the standing commission in `operating-cadence.md` at `none`, so the ordinary stance applies and no commission is cleared by this change set. The subject test fails for witness and correctly: deciding whether a branch of his own still has a fixture on it is a move he has.

**Register: generic present throughout.** No event is asserted as having happened, no first person appears, and no composite stands in for one. The trade specifics — dead legs, cutting back to the main, disinfectant residual decaying in standing water — are ordinary potable-water practice used as metaphor.

**Imagery: three new photographs, generated and reviewed, none reused.** Generated in Chrome through the pinned **Grown Men Grow** project, continuing the most recent *Editorial Photography Request* thread as the fourth, fifth, and sixth images in it — the style is established and only the subject is new. `capped-copper-stub-wall`, `basement-main-run-tees`, and `tubing-cutter-on-copper`, each 1024×1536, each reviewed at full resolution against the believability gate before it entered the repository. Nothing was regenerated; all three passed on the first attempt. The exact prompts are recorded in the draft's production notes.

**One judgement call worth flagging.** `basement-main-run-tees` has a water heater in it as set dressing, and Field Note 17 is about an anode rod in a water heater. The exclusivity ruling is about photographs rather than motifs, and this is a different photograph of a different room at a different distance — but a founder who wants no echo at all should say so, because it is a call an agent made rather than a rule that decided it.

**The signature mark went through one revision before it was legible.** A branch-stub mark: a main run, a tee, a short branch closed by a filled cap, with the standing water drawn inside it. The first version hatched the branch on a diagonal and the four ticks read as a spring — motion, which is the opposite of what is being drawn. They are level rungs now. The stroke weights were raised and the cover and slide-4 photographs re-anchored (`xMidYMax` and `xMidYMin`) because the default centre crop put the stub and the ceiling tees outside their windows.

**Gates run, all green:** `verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` plus GScan over the packaged zip, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'`, 128 passing; `verify-repository.mjs`, now reading 631 tracked files, 23 review sheets and 64 editorial sources; `verify-svg-xml.sh`, 190 files; and `git diff --check` clean on the full staged list.

**Files changed:** `drafts/field-note-19-the-line-was-capped-not-removed.md` and `drafts/field-note-19-platforms.md` (new); `scripts/render-field-note-19.mjs` (new); `assets/source/editorial/{capped-copper-stub-wall,basement-main-run-tees,tubing-cutter-on-copper}.png` (new); `assets/drafts/instagram/field-note-19-carousel/` and `assets/drafts/ghost/feature-images/the-line-was-capped-not-removed.{png,svg}` and `assets/drafts/review/field-note-19-carousel.png` (generated); `scripts/verify-repository.mjs` and `scripts/render-review-contact-sheets.mjs` (registration); this log.

**External state changed:** none in the publication. Nothing was published, sent, scheduled, posted, or purchased, and no account setting was touched. Three image generations were run in the founder's own ChatGPT project, which is the only external system this run wrote to.

**Open, in order:** (1) **Field Note 19 needs a founder read** — essay, pack, and artwork in one pass; nothing moves to `content/` or takes a register slot without it. (2) The water-heater echo above is a founder call if it matters. (3) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`, and task instruction files drifting out of step with the repository with nothing detecting it.

## 2026-09-09 — Claude Code: Field Note 19 approved and moved into content; two dependency advisories cleared

**Client:** Claude Code (same session as the entry above, founder present for this part). **Branch:** `claude/field-note-19-draft`, PR #168. This entry follows the preceding one, which recorded Field Note 19 as an unapproved draft awaiting a founder read. That is no longer true. The earlier entry stands as written, per the append-only rule.

**The founder approved the work as delivered and it moved to `content/`.** Essay to `content/field-notes/the-line-was-capped-not-removed.md`, pack to `content/distribution/field-note-19-platforms.md`, both by `git mv` so the history follows. Frontmatter now reads `status: founder-approved` with `approved: 2026-09-09`. **`publication_authorized` stays `false` on both files and no Ghost slot was named** — the approval covers the essay, the pack and the artwork, which is the same shape as Field Notes 14 and 18. It takes register position 18 in `publication-order.md`, dated 2026-12-15 as a projection, and `drafts/README.md` records the move. A guard in `verify-repository.mjs` now fails if either file reappears under `drafts/`, matching the guards standing for Notes 2, 12 and 13.

**The generation prompts and source paths are recorded in the note's production notes**, per the visual system's requirement, along with the water-heater echo in `basement-main-run-tees` that was raised before approval and accepted.

**PR #168 was blocked by the dependency gate, on a failure that had nothing to do with this change set.** `main` was green on the 2026-09-08 push and on that day's scheduled sweep; two advisories published between then and this run, so the block was a fresh regression on the default branch that this branch simply inherited. Both are in `theme/`, both devDependency-only, and neither reaches the published site.

**multer 2.2.0 was fixed rather than accepted.** Four advisories — GHSA-535w-7cp7-47q4, GHSA-qfvm-cv95-jqjf, GHSA-qvfw-j98x-7q72, GHSA-wc9g-mqfw-jrwm — all fixed in 2.3.0, which the scanner itself reported as fixable. gscan 6.4.2 pins multer to **exactly** 2.2.0, so no lockfile refresh could move it and an override was the only route. **The first attempt silently did nothing**: `pnpm.overrides` was written into `theme/package.json`, and pnpm 11 no longer reads that field — it warns and ignores it. That is the worst class of fix, because the repository would have looked corrected while the lockfile still resolved 2.2.0. The override lives in `theme/pnpm-workspace.yaml` now and `pnpm why multer` confirms 2.3.0 resolved. GScan passes on both the theme and the packaged zip with the newer multer.

**A near-miss worth recording, because nothing would have caught it.** `theme/pnpm-workspace.yaml` already existed and already carried `allowBuilds: dtrace-provider: false`. It was overwritten wholesale by a heredoc before the file was read, which would have silently re-enabled a native compile of dtrace-provider on every CI install. It was caught only because an `ls ... || echo` in the same command printed the path instead of the fallback. **The setting is restored and the file's diff is purely additive — 21 insertions, zero deletions.** The rule that failed here is the ordinary one: look at the target before overwriting it.

**extract-zip GHSA-7pqw-9j4j-h8q3 (High 8.1) was accepted, not fixed, and that is a decision worth naming.** It is a second symlink-escape in extract-zip 2.0.1 — containment checked against an entry's parent directory but never its own final path component — and the advisory was read before the rationale was written rather than assumed from the first one. It has no patched release and no upstream path off it, on facts identical to the already-accepted GHSA-jmr9-qjv8-65gv: gscan 6.4.2 pins @tryghost/zip 3.5.0 exactly, and 3.5.6 still pins extract-zip 2.0.1 exactly. The exposure argument is the one that carries it: the attacker must control the archive, and the only archive GScan ever extracts is the theme zip this repository builds moments earlier from its own tracked files. **It expires 2026-11-09, deliberately the same day as the first entry**, so both come up as one re-decision rather than two chances to forget one.

**Verified rather than assumed:** `osv-scanner scan source --lockfile theme/pnpm-lock.yaml --config osv-scanner.toml` was run locally and reports `No issues found`, with both extract-zip advisories filtered by name and reason and no multer finding at all.

**Gates run, all green:** `verify-ghost-theme.mjs`; `pnpm --dir theme install --frozen-lockfile`; `pnpm --dir theme test`; `pnpm --dir theme zip` plus GScan over the packaged zip, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'`, 128 passing; `verify-repository.mjs`; `verify-svg-xml.sh`, 190 files; `git diff --check` clean; and the OSV scan above.

**Files changed:** `content/field-notes/the-line-was-capped-not-removed.md` and `content/distribution/field-note-19-platforms.md` (moved from `drafts/`, frontmatter and production notes updated); `docs/technical/publication-order.md` (position 18); `drafts/README.md`; `scripts/verify-repository.mjs` (drafts guard); `theme/pnpm-workspace.yaml`; `theme/pnpm-lock.yaml`; `osv-scanner.toml`; this log.

**External state changed:** none. Nothing was published, sent, scheduled, posted, or purchased, and no account setting was touched. Approval moved a file; it did not put anything in front of a reader.

**Open, in order:** (1) **Field Note 19 holds register position 18 with no Ghost slot** — publication timing is still a founder call, unchanged by approval. (2) **The extract-zip acceptance expires 2026-11-09 and both entries fall due together**; if gscan or @tryghost/zip has moved by then, the fix is an upgrade rather than a second renewal. (3) The multer override should be removed once gscan ships a release pinning 2.3.0 or later, and nothing currently watches for that. (4) The bank is now eighteen pieces against a corpus-test threshold of twenty-one, and four of the eighteen are `witness` — two more approved assignment notes make the next witness commission the live question. (5) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`, and task instruction files drifting out of step with the repository with nothing detecting it.

## 2026-09-09 — Claude Code: the two dependency items closed at the root, and the staleness itself mechanized

**Client:** Claude Code (same session, founder present). **Branch:** `claude/dependency-exemption-premises`. This closes two of the four items left open by the entry above. The other two are not agent decisions and are left standing with reasons.

**What the previous entry got wrong, found by checking instead of assuming.** It reported the multer override as a fix needing a future watch, and the extract-zip acceptance as resting on stated facts. Re-deriving those facts from the registry showed the rationale had been asserting three things that were no longer true: gscan 6.4.2 was not "the current release" (6.6.0 was), `@tryghost/zip` 3.5.6 was not "its latest" (3.5.12 was), and the current `@tryghost/zip` pins extract-zip as `^2.0.1` rather than exactly. **None of that changed the conclusion — extract-zip still has nothing above 2.0.1 — but the acceptance had been standing on stale ground for weeks and nothing could tell.**

**The multer override is gone, fixed at the root.** gscan 6.5.0 pins multer 2.3.0 itself, so the override had no reason to exist. `theme/package.json` moves 6.4.2 → 6.5.0, `pnpm why multer` reports 2.3.0 with no override in play, and gscan passes on both the theme and the packaged zip. Item three from the previous entry is closed rather than watched: there is no workaround left to outlive its reason.

**gscan 6.6.0 was deliberately not taken.** It drops multer entirely, which is tidier, but it was **seventeen hours old** and pnpm's own cooling-off guard refused it — the install auto-wrote a `minimumReleaseAgeExclude` waiver into `pnpm-workspace.yaml` to get past its own protection. Waiving a supply-chain guard to take a same-day release into a required CI gate is not a trade worth making for tidiness. 6.5.0 is nearly six days old, clears the guard with no waiver, and already carries the patched multer. The waiver was removed and no exclusion remains in the file.

**The durable fix is `scripts/verify-dependency-exemptions.mjs`, and it is the point of this change set.** Expiry dates already force a re-decision on a fixed day. They cannot catch a premise dying inside the window, which is exactly what happened here. The script reads no rationale at all: it derives the advisory population from `osv-scanner.toml`, asks OSV what each advisory affects, asks the npm registry what has actually shipped, and fails when a patched release exists, when an expiry has passed, or when it could not reach either service. That last case is deliberate — a premise that was not checked is not a premise that holds, and passing there would be the silent failure `AGENTS.md` forbids. It also uses the registry as a second source, so a patch that ships before OSV records it is still caught.

**It was proved failing before it was trusted.** A gate never seen to fail is not known to work, so a temporary entry accepting `GHSA-535w-7cp7-47q4` — a multer advisory that *is* patched — was added to the policy file and the script exited 1 naming multer 2.3.0 and telling the operator to upgrade rather than renew. The entry was then removed and the check returns to reporting both real acceptances as holding. Fifteen unit tests cover the parsing and verdict logic, including the empty-population case, a prerelease not counting as a patch, and the expiry boundary day.

**The rationales no longer restate version numbers**, because that is the thing that rotted. They now state why the risk is tolerable — devDependency only, and the sole archive GScan extracts is the zip this repository builds from its own tracked files — and leave every version fact to the checker. `AGENTS.md` records that rule so the next accepted risk is written the same way.

**Two items are left open on purpose.** Field Note 19's Ghost slot is a founder call under the launch-authority rule and no agent should take it. The witness/corpus question is already mechanized — `verify-repository.mjs` enforces the corpus threshold and the `**Standing commission:**` marker is machine-read by the preflight — and setting that marker to `witness` is a deliberate editorial instruction about the next draft, which is the founder's to give.

**Gates run, all green:** `verify-ghost-theme.mjs`; `pnpm --dir theme install --frozen-lockfile`; `pnpm --dir theme test` and GScan over the packaged zip under the new gscan, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'`, now 143 passing; `verify-repository.mjs`, 634 tracked files and 51 JavaScript files; `verify-svg-xml.sh`, 190 files; `verify-dependency-exemptions.mjs` against live OSV and npm; `osv-scanner` locally reporting `No issues found`; `git diff --check` clean.

**Files changed:** `scripts/lib/dependency-exemptions.mjs`, `scripts/test/dependency-exemptions.test.mjs`, `scripts/verify-dependency-exemptions.mjs` (new); `scripts/verify-repository.mjs` (all three registered in the manifest); `.github/workflows/security.yml` (new `exemption-premises` job on every trigger the OSV scan takes); `AGENTS.md` (the `release:` gate, the workflow description, and the rule about how a rationale is written); `osv-scanner.toml`; `theme/package.json`; `theme/pnpm-lock.yaml`; `theme/pnpm-workspace.yaml`; this log.

**External state changed:** none. Nothing published, sent, scheduled, posted, or purchased; no account setting touched.

**Open, in order:** (1) **Field Note 19 holds register position 18 with no Ghost slot** — a founder call. (2) **The witness commission is a founder call**; the bank is eighteen with four witness pieces and the corpus threshold is twenty-one, all of it already enforced. (3) The extract-zip acceptances still expire 2026-11-09, and now the premise under them is re-derived daily rather than trusted until that date. (4) `exemption-premises` is not a required status check; it fails the run but does not block a merge on its own. Making it required is a ruleset change and belongs with the next fleet conformance pass. (5) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`, and task instruction files drifting out of step with the repository with nothing detecting it.

## 2026-09-09 — Claude Code: `Exemption premises` made a required check; it was never failing

**Client:** Claude Code (same session, founder present). **Branch:** `claude/require-exemption-premises`. This closes open item (4) from the entry above, which deferred it to "the next fleet conformance pass".

**First, a correction to how the previous entry read.** It said `exemption-premises` "fails the run but does not block a merge on its own", meaning *when it detects a problem* it fails the run. Read quickly that says the job is failing. **It is not and never has been.** It is green on every run since it was introduced — the `claude/dependency-exemption-premises` pull request, both `main` pushes since, and a local run at 11:20 ET — and it reports both extract-zip acceptances as still holding. The wording was mine and it was ambiguous; the job's record is clean.

**The real gap was that it was not a required status check.** `main-requires-green-ci` required four contexts — `Dependency scan / osv-scan`, `Repository verification`, `Secret scan`, `Semgrep CE` — and the job added yesterday was not among them, so a run where it detected a dead premise would have gone red beside a mergeable pull request. The global contract is explicit that required status checks are "that repo's real CI job names, **all of them**", so this was a conformance defect from the moment the job landed, not a preference.

**The population was derived rather than trusted.** Every job across all four workflow files was enumerated and compared against the ruleset. Exactly one addition was warranted. The three jobs left out are correctly out: `review` from `claude-review.yml` is advisory and skips without the credential, `dependabot-auto-merge` is barred by name from ever becoming required, and `Ghost managed edge` carries `if: github.event_name != 'pull_request'` so requiring it would block every merge permanently.

**The ruleset now requires five contexts.** The change was additive and verified from the server rather than from the payload sent: `required_linear_history` and `non_fast_forward` are intact, `bypass_actors` is still empty, enforcement is still `active`, and no previously required check was dropped — the build script asserts that last point rather than assuming it, because the standard says converting a repo never drops a check it already required. `/tmp` held a pre-change snapshot for rollback throughout.

**This class was already mechanized; it simply had not run yet.** `scripts/fleet-conformance.sh` in windwardline/windwardline derives job names from real workflow runs and emits `unrequired-job:<name>` for any non-skipped job missing from the required contexts, so the weekly fleet-health cadence would have caught this on Monday. Fixing it now rather than waiting is the difference between a gate that binds today and one that binds in four days. **The durable answer to "how do we stop this recurring" is that the mechanism already exists and is correct — the failure mode is the gap between adding a job and the checker next running, and the fix for that is to add the required check in the same change set that adds the job.**

**A second staleness was found and removed while here.** `AGENTS.md` said "One entry is live: extract-zip 2.0.1 (GHSA-jmr9-qjv8-65gv)" — written when that was true and left standing after a second acceptance was added yesterday. That is precisely the rot the `exemption-premises` job exists to catch, reproduced inside the paragraph describing it. The contract no longer restates the register's contents at all; it points at `osv-scanner.toml` and says to read it. The same sentence also now records that the job is required, runs unguarded on every pull request including Dependabot's, and fails closed on an OSV or npm outage.

**Gates run, all green:** `verify-ghost-theme.mjs`; `node --test 'scripts/test/**/*.test.mjs'`, 143 passing; `verify-repository.mjs`, 634 tracked files; `verify-svg-xml.sh`, 190 files; `verify-dependency-exemptions.mjs` against live OSV and npm; `git diff --check` clean.

**Files changed:** `AGENTS.md`; this log.

**External state changed:** **the `main-requires-green-ci` branch ruleset on windwardline/grown-men-grow now requires five status checks instead of four.** This makes the gate stricter and cannot loosen it. Nothing was published, sent, scheduled, posted, or purchased.

**Open, in order:** (1) **Field Note 19 holds register position 18 with no Ghost slot** — a founder call. (2) **The witness commission is a founder call**; bank eighteen, four witness, threshold twenty-one, all enforced. (3) The extract-zip acceptances expire 2026-11-09 with their premise now re-derived daily. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended until thirty recipients, `publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`, and task instruction files drifting out of step with the repository with nothing detecting it.

## 2026-09-09 — Claude Code: two carried-forward items mechanized, and one of them was a live defect

**Client:** Claude Code (same session, founder present). **Branch:** `claude/mechanize-carried-items`. The founder's correction was that this session kept reporting issues rather than fixing them durably. That was accurate, and this entry closes the two items that were being carried forward rather than solved.

**A carried-forward line was itself wrong, which is the whole point.** Every recent entry has ended with "`publication-order.md` row 3 still reading `scheduled` for `friendship-has-a-maintenance-schedule`". Row 3 has read `published` for some time. The line was copied from entry to entry without being checked — reporting a defect that no longer existed while a real one sat two rows below it.

**The real defect: row 4 read `scheduled` for a note Ghost published the previous morning.** `rest-is-not-a-reward` went out 2026-09-08 and the register was never updated. This is not cosmetic. The Monday staging task takes the lowest-numbered note with no Ghost post, so the State column is the input to the decision about what publishes next, and nothing could detect it being wrong because the only record of what had happened was the column that was wrong. Row 4 now reads `published`, verified against Ghost rather than assumed.

**`scripts/verify-publication-register.mjs` reconciles all nineteen rows against Ghost.** It parses the table rather than holding a list, refuses a State value it cannot read, refuses a slug listed twice, and — deliberately — fails on a row Ghost could not be asked about rather than treating a failed lookup as an absent post. It is a `cadence:` gate because it needs the Ghost admin key from the local Keychain, which CI does not have; its home is the Monday staging task, before that task decides anything. Proved both ways: reverting row 4 made it exit 1 naming the note and saying the register was never updated, and the row was restored immediately.

**`scripts/verify-required-checks.mjs` closes the gap this session created yesterday.** `exemption-premises` was added to `security.yml` without a ruleset entry and nothing caught it for a day. The previous entry's answer to "how do we stop this recurring" was a sentence in `AGENTS.md` telling the next agent to remember — which is exactly the failure this repository's standards name, a brake that lives in prose. It is now a required CI job that reads the branch's rules and reconciles them against the workflow files in both directions: a job running on pull requests that is not required fails, and a required context no job produces fails too, because a check that never reports never passes and would block every merge forever.

**It reads `/repos/{owner}/{repo}/rules/branches/{branch}` rather than `/rulesets`,** because the admin-scoped endpoint is not readable by a workflow's `GITHUB_TOKEN` and a check that cannot run is not a check. Three jobs do not gate, each for a stated reason rather than by omission: `review` is advisory and skips without its credential, `dependabot-auto-merge` arms auto-merge rather than gating it, and `Ghost managed edge` needs no exemption at all — the parser derives from its `if: github.event_name != 'pull_request'` guard that it never reports on a pull request.

**Proved failing before it was trusted.** A temporary `Temp proof job` was appended to `ci.yml` and the check exited 1 naming it and explaining that an unrequired job blocks nothing; the file was restored immediately and `git diff` confirms it clean. That is the same defect shape as yesterday's, reproduced on demand.

**The new job was added to the ruleset in the same change set, which is the discipline the check now enforces.** `Required checks` is itself a pull-request job, so by its own rule it must gate. The ruleset went from five required contexts to six, applied additively and verified from the server: `required_linear_history` and `non_fast_forward` intact, `bypass_actors` still empty, enforcement still `active`, no previously required check dropped.

**Gates run, all green:** `verify-ghost-theme.mjs`; `pnpm --dir theme test` and GScan over the packaged zip, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'`, now 168 passing (25 new); `verify-repository.mjs`, 640 tracked files and 57 JavaScript files; `verify-svg-xml.sh`, 190 files; `verify-dependency-exemptions.mjs`; `verify-required-checks.mjs`; `verify-publication-register.mjs` against live Ghost; `git diff --check` clean.

**Files changed:** `scripts/lib/required-checks.mjs`, `scripts/test/required-checks.test.mjs`, `scripts/verify-required-checks.mjs`, `scripts/lib/publication-register.mjs`, `scripts/test/publication-register.test.mjs`, `scripts/verify-publication-register.mjs` (new); `scripts/verify-repository.mjs` (all six registered); `.github/workflows/security.yml` (`required-checks` job); `AGENTS.md` (two declared gates and both mechanisms described); `docs/technical/publication-order.md` (row 4); this log.

**External state changed:** **the `main-requires-green-ci` ruleset now requires six status checks instead of five.** Stricter, not looser. Nothing published, sent, scheduled, posted, or purchased.

**Open, in order:** (1) **Field Note 19 holds register position 18 with no Ghost slot** — a founder call under launch authority, and the only thing genuinely waiting on a person. (2) **The witness commission is a founder call**; bank eighteen, four witness, threshold twenty-one, all enforced. (3) The extract-zip acceptances expire 2026-11-09 with their premise re-derived daily. (4) **The remaining carried-forward items are not agent-fixable and should stop being listed as though they were**: the distribution problem and the newsletter list at one member are outcomes of publishing, not defects, and the A/B stays suspended until thirty recipients by its own marker. (5) Task instruction files under `~/.claude/scheduled-tasks/` still drift from the repository with nothing detecting it — genuinely unmechanized here, because those files live outside this repository and no check inside it can see them; the honest home for that is the `ops` repo, not this one.

## 2026-09-12 — Claude Code: weekly analytics readout; the engagement numbers are the house's own accounts

**Client:** Claude Code, scheduled task `gmg-friday-analytics`, run unattended. **Branch:** `claude/analytics-2026-09-12`. Verification only. Nothing was published, sent, posted, scheduled, replied to, or purchased, and no account setting was touched.

**The task itself fired about fifteen hours late**, at 00:43 ET Saturday against a Friday slot. That is the mechanism recorded in `publish-timing.md` — a scheduled task is real only while the desktop application is open — and Friday remains its worst day. It is logged here rather than as a new finding because it is the fourth consecutive Friday observation of the same thing.

**Ghost, week of 2026-09-05.** One post: `rest-is-not-a-reward`, published 2026-09-08 at 08:00 ET, exactly on slot. Email delivered 1, opened 1, failed 0. The list is **one member**, unchanged for the fifth consecutive week — joined 2026-08-11 and still the only one. Open rate is 100% and it is not a measurement; the A/B stays suspended under its own thirty-recipient marker. `/stats/` and `/links/` were probed rather than assumed and still answer 403 to an integration key, so 48-hour pageviews and per-post conversions remain uncollectable programmatically.

**Every cross-platform slot fired on time**, which is worth stating because the Tuesday fragment missed entirely on 2026-09-01. Bluesky fragment Tuesday 12:00 ET (+2 min), LinkedIn Wednesday 10:00 ET (+2 min), Instagram carousel Thursday 09:00 ET (+3 min), Bluesky link Saturday 06:30 PM ET (+9 sec). All four are Buffer-scheduled and fire server-side; the slots that miss are the ones this machine has to be awake for. That distinction is the real variable and it is sharper than the weekday/weekend framing the Saturday section reached for.

**Per-post engagement, pulled from Buffer's `metrics` field.** Instagram Thu 09-10: reach 3, views 11, reactions 3, comments 0, saves 0, shares 0, follows 0. LinkedIn Wed 09-09: reach 6, impressions 6, reactions 0, comments 0. Bluesky Tue 09-08 and Sat 09-05: reactions 0, reposts 0, replies 0 on both.

**Audience sizes, read from each platform rather than inferred.** Instagram 4 followers. Bluesky **0 followers**, 9 posts, zero replies or reposts across all nine, and two likes total — both on the first post, 2026-08-11. LinkedIn **0 followers**, 2 post impressions and 0 page visitors in the last seven days. Substack 1 subscriber, 5 notes, likes between zero and two. Ghost 1 member.

**The finding that matters: the Instagram engagement is the house's own accounts.** Business Suite shows 14 content interactions on 09-10, the only real spike in the 28-day window and roughly half its total of 31. The Instagram notification history explains it — `windwardline`, the parent-brand account, liked eight posts in one pass, and the remaining likes come from `grownmengrow` itself through the linked Accounts Center and from `metsan_kuningas`, one of the four followers. **Read as audience signal those numbers would be badly misleading, and a readout that quoted the 933% interactions delta without opening the notification feed would have reported growth that did not happen.** Reach is the honest column: 6 for the week, 21 across 28 days.

**MODERATION SWEEP — nothing on any surface, and nothing escalatable.** No comment, message, reply, or mention exists anywhere to apply `community-moderation.md` to. Ghost: the admin comments endpoint returns `total: 0` across the whole publication, not merely across recent posts. Instagram: the full notification history back to 2026-08-07 contains **no comment notification of any kind**, and Primary, General, Requests and Hidden Requests message folders are all empty — Hidden Requests being the folder where an offensive or filtered DM would land. Bluesky: 0 replies and 0 quotes on all nine posts, read from the public AppView. LinkedIn: no comments on any page post. Substack: no replies on any note.

**The Meta Business Suite inbox failed to render for the second consecutive readout.** The thread list stays on skeleton placeholders past nineteen seconds on both All Messages and Instagram Comments, and clicking the tab does not change the selection. That is a repeat of the 2026-09-05 blocker rather than a transient. **It did not cost the sweep anything this week** — the Instagram evidence above came from three independent routes that do not go through that panel, and Business Suite's own to-do list separately reported nothing awaiting attention. It is recorded because the day a real message arrives, the panel the playbook names is the one that does not load.

**Qualitative pass: there is nothing to report, and that is the report.** No reader has said anything back on any surface, so there is no recurring question, no objection, and no disclosure to handle carefully. Principle 5 asks how the work landed; the honest answer after five weeks of publishing is that it has not yet reached anyone who was not already in the house.

**Corpus balance check: skipped, correctly, and not due.** The marker in `decision-log.md` reads 2026-08-26 at a bank of 15, next due at 21. The bank is **19** — 15 `assignment` and 4 `witness` — so it is two pieces short. The marker was not advanced, because recording a measurement that was not made is the thing it exists to prevent. For context only, not as a measurement: the fourth column is at 4 of 19 and is not disappearing, and of the seven pieces whose closing does not turn to the reader, four are the witness pieces where that is required.

**The register was reconciled against Ghost** with `verify-publication-register.mjs` — all 19 rows agree, 5 published, 0 scheduled, 14 projected. **Nothing is currently scheduled on Ghost for the 2026-09-15 slot**; that is the Monday staging task's job and row 5 (`you-cant-outwork-a-wrong-direction`) is the note it will take.

**Verification:** `verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` plus GScan over the packaged zip; `node --test 'scripts/test/**/*.test.mjs'`; `verify-repository.mjs`; `verify-svg-xml.sh`; `verify-publication-register.mjs` against live Ghost; `git diff --check` clean.

**Files changed:** this log.

**External state changed:** none.

**Open, in order:** (1) **The distribution problem is now measured rather than asserted** — three of five surfaces have zero or one follower after five weeks, and the only engagement recorded is internal. This is a founder call about reach, and no slot-tuning reaches it. (2) **Field Note 19 holds register position 18 with no Ghost slot** — a founder call under launch authority. (3) **The witness commission is a founder call**; bank 19, four witness, corpus threshold 21. (4) The Business Suite inbox render failure is now a repeat; if it persists a third week the playbook should name a route that works instead of one that does not. (5) The extract-zip acceptances expire 2026-11-09 with their premise re-derived daily.

## 2026-09-12 — Thursday Medium import: "Rest Is Not a Reward" published, and the tag trap is not specific to one tag

**Client:** Claude Code, scheduled task `gmg-thursday-medium`, run unattended. **Branch:** `claude/thursday-medium-import-2026-09-12`.

**The story is live on Medium:** https://grownmengrow.medium.com/rest-is-not-a-reward-e34fb3d52f92

**The slot was missed on Thursday and recovered about forty-two hours late.** The run began at 00:44 ET on Saturday 2026-09-12 against a Thursday 06:45 ET slot, in the same launch catch-up burst that fired `gmg-friday-analytics` a minute earlier. As recorded on 2026-08-30, this task needs no lateness guard: its binding constraint is the canonical-first rule rather than the clock, and a syndicated copy carries no time-of-day signal the way a social post does. Being late means Thursday was missed, not that executing now is wrong. Stated plainly so a two-day gap does not read as an on-time run.

**Preconditions, each checked rather than assumed.** `latestPublishedPost()` returned `rest-is-not-a-reward`, published 2026-09-08T12:00:00Z — 88.7 hours before the run against a 40-hour minimum. `https://grownmengrow.com/rest-is-not-a-reward/` returned 200. Both Medium surfaces were checked because the profile feed lists published stories only and a stale import draft is invisible to it: Drafts read "No stories in draft," and Published held exactly the four prior stories — *Friendship Has a Maintenance Schedule* (Sep 1), *A Confession Can Still Be Selfish* (Aug 25), *Call Your Friends Before There's a Reason* (Aug 18), *Strength Has to Grow Up* (Aug 9). That count agrees with the 2026-09-03 entry, so nothing landed between runs.

**The importer took the SEO title for the third consecutive week, exactly as predicted.** It imported as *Rest Is Not a Reward | Grown Men Grow* — Medium reads the page `<title>` and Ghost appends the site suffix to the derived `meta_title`. Corrected to the post's `title` field, *Rest Is Not a Reward*. The 2026-09-08 entry called this a standing consequence of the derivation rather than a divergence to fix, and that reading held.

**The two destructive edits this task has recorded did not recur, because the method changed.** `Home` and `End` are document-scoped in Medium's contenteditable, which is what destroyed a title on 2026-08-30 and 5,099 characters of body on 2026-09-03. Neither key was pressed this run. Both edits were made by clicking to place a caret, **reading back from the DOM that the selection was collapsed at a known offset inside the intended block**, and then pressing a counted number of BackSpaces. That has a bounded blast radius of exactly N characters and cannot reach another block. The title went from 37 characters to 20 with 17 presses; the `Grown Men Grow 2 min read` line went from 25 characters to empty with 25, and one further press removed the emptied block. Block count went 28 → 27 and the paragraph above the removed line still measured 44 characters afterwards, so nothing merged or was lost.

**The preview image did not import empty this week, which the task file says it always does.** It arrived already set from the story's feature image — alt *Rest Is Not a Reward*, an 880×550 crop served from `miro.medium.com` — so the "Change preview image" step was not needed and was not taken. Recorded because the task file states the empty import as unconditional; on this run it was conditional, and a future run should check the field rather than assume it needs filling.

**The tag autocomplete trap is general to short tags, not specific to "Men."** The task file and the 2026-09-03 entry both name `Men` as the case in hand. `Men` behaved exactly as documented — suggestions ran *Mental Health (9.6M)*, *Mental Health Awareness (3K)*, *Men (1.4K)*, so a blind Return commits *Mental Health*. **`Rest` has the same shape and is not recorded anywhere:** suggestions ran *Restaurant*, *Rest Api (2.5K)*, *Rest (264)*, *Restaurant Business (269)*, *Django Rest Framework (2.4K)*. A blind Return would have tagged this essay **Restaurant**. Both correct tags sat third. The rule worth carrying is the general one — any tag short enough to be a prefix of a larger topic will be outranked by it — rather than a list of the two tags that have hit it so far.

**Every tag was committed by highlight-then-Return and then read back.** Type the tag, press Down until the exact match is highlighted, confirm the highlight on screen, then Return. `Masculinity` offered a single suggestion and the arrow key produced no visible highlight change; Return was still safe there because the typed string and the only option were the same string, and the committed chip was verified either way. The five chips read back from the dialog as **Men, Rest, Burnout, Masculinity, Personal Growth** — exactly the set in `content/distribution/field-note-07-platforms.md`, in pack order.

**The canonical was confirmed before publishing by reading the DOM property, not the pixels.** A disabled input renders a real `value` and a `placeholder` in identical grey, which is why the task file warns about it. The field reported `disabled: true`, a `value` of 46 characters equal to `https://grownmengrow.com/rest-is-not-a-reward/`, and a separate 25-character placeholder — so the greyed URL was a stored value. "This story was originally published elsewhere" was checked. Story settings also showed the author as **Grown Men Grow (@grownmengrow)** with no founder name anywhere.

**Verified after publication from server-delivered source rather than the rendered DOM.** A `fetch` issued inside the page context returned HTTP 200 and 151,202 bytes carrying `<link rel="canonical" href="https://grownmengrow.com/rest-is-not-a-reward/"/>`. Run in the browser, never with `curl`, which Medium answers 403 — an unread exit status there turns a refused request into a false pass. The live page's `h1` reads *Rest Is Not a Reward*, the imported min-read line is absent, and the five tag links are the pack's five.

**No reshare.** The post-publish dialog offered Facebook, LinkedIn and X; none was touched and it was dismissed with its close control. Other platforms receive reviewed native copy, never a Medium reshare.

**One observation for the founder, not a defect.** The live Medium subtitle is the note's `dek` — *You don't earn maintenance. You schedule it.* — which the importer lifts from the Ghost article header. The pack's Medium section names a different subtitle, *Collapse is not rest. It's running out.* Both strings are founder-approved and the importer set the dek automatically; the four prior imports have the same shape. Whether the pack's line should replace it on Medium is an editorial call, and no approved copy was altered to force one.

**The publication register needed no change.** `docs/technical/publication-order.md` row 4 already reads `published` for this note, corrected in the 2026-09-09 change set that added `verify-publication-register.mjs`.

**A concurrency note, because the working tree was shared.** `gmg-friday-analytics` fired a minute before this task and had an uncommitted analytics entry sitting in `docs/technical/handoff-log.md` when this run reached its own log step. That entry was neither staged nor modified. This commit was built from `origin/main` with only this entry appended, so the analytics readout stays with its own pull request rather than being smuggled into this one — the same resolution the 2026-08-24 entry records for the same collision.

**Gates run, all green:** `verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` plus GScan over the packaged zip, no fatal issues on Ghost 6.x; `node --test 'scripts/test/**/*.test.mjs'`; `verify-repository.mjs`; `verify-svg-xml.sh`; `git diff --check` clean.
...
## 2026-09-12 — Field Note 20 drafted complete: "The Opener Doesn't Lift the Door" (witness)

**Client:** Claude Code, the `gmg-saturday-draft` scheduled task, founder not present. **Branch:** `claude/saturday-note-20-2026-09-12`, cut from `origin/main` at 957033a. One complete unit was produced — essay, platform pack, three photographs, render script, carousel, feature image, review sheet, and gate entries — and **none of it is founder-approved or authorized for publication.** It lives in `drafts/`.

**Backfill was checked first and none was owed.** Every note in `content/field-notes/` was measured against the complete-unit table in `docs/technical/content-pipeline.md`: eighteen notes, eighteen packs in `content/distribution/`, eighteen render scripts, eighteen carousels, and a review-sheet entry apiece. Nothing was missing, so the run proceeded to new work rather than consuming itself on repair.

**The stance is `witness`, and it was chosen on the standing corpus duty rather than on a commission.** `operating-cadence.md` carries `**Standing commission:** none`, so no commission line is cleared by this change set and the ordinary `assignment` value was available. It was not taken, because witness sat at four pieces in nineteen — below the roughly one quarter the Stance section describes — and the two most recent notes, Field Notes 18 and 19, both shipped `assignment`. The subject test was applied rather than assumed: the counterbalance spring is wound before he arrives, to a turn count he did not choose, and he can neither add cycles back nor re-wind it himself, so there is no move available to him inside the event.

**It is deliberately not a sibling of Wednesday's.** Field Note 19 is `assignment`, opens on a code requirement, and closes by turning to the reader. This one is `witness`, opens on an arithmetic mismatch between a motor and a door, and closes on the last fact with no imperative and no question.

**All nine tests from `docs/editorial-underpinning.md` were run and reported in chat, with the two near-misses named rather than sanded off.** Test 2 (the pulled punch) and test 7 (who it lands worst on) are recorded in the delivery, along with the one-spring passage that answers test 7 and the mis-sized-spring passage that keeps the piece from certifying that the care landed well.

**Files changed:** added `drafts/field-note-20-the-opener-doesnt-lift-the-door.md`, `drafts/field-note-20-platforms.md`, `scripts/render-field-note-20.mjs`, three source photographs under `assets/source/editorial/`, seven carousel SVG/PNG pairs under `assets/drafts/instagram/field-note-20-carousel/`, the feature image pair under `assets/drafts/ghost/feature-images/`, and `assets/drafts/review/field-note-20-carousel.png`; modified `scripts/verify-repository.mjs` (required file, asset family, feature-image count 21→22, three editorial sources, review sheet) and `scripts/render-review-contact-sheets.mjs` (one sheet entry), plus this log.

**A concurrent session held the primary checkout, and this run did not commit its work.** `/Users/peacock/Projects/grown-men-grow` carried another session's untracked `substack-*` scripts, tests and doc together with its own additions to `requiredFiles` in `scripts/verify-repository.mjs` — the same file this run edits. Rather than stage a shared file carrying both sets of hunks, this run created a separate git worktree at `origin/main`, copied only its own artifacts in, and re-applied its five edits to a clean `verify-repository.mjs` there. The neighbour's working tree was left exactly as found; `git status` in the primary checkout still shows its files untracked and its hunks unstaged. **Every gate below was therefore run in the clean worktree, against the staged tree that is actually being committed, not against a working tree holding a second session's half-finished work.**

**Verification, each gate named and run, all green:** `node scripts/verify-ghost-theme.mjs` (17 theme files); `pnpm --dir theme install --frozen-lockfile` then `pnpm --dir theme test`; `pnpm --dir theme zip` with `theme/node_modules/.bin/gscan -z --fatal --verbose dist/grown-men-grow.zip`; `node --test 'scripts/test/**/*.test.mjs'` (168 tests, 0 failures); `node scripts/verify-repository.mjs` (663 tracked files, 187 PNG/SVG pairs, 24 review sheets, 67 editorial sources); `bash scripts/verify-svg-xml.sh` (198 SVG files); plus `git diff --check`, `git diff --cached --check`, and a reviewed staged file list.

**Photography: three images, generated and reviewed, none regenerated.** Produced in the founder's ChatGPT project, continuing the current *Editorial Photography Request* thread as its seventh, eighth and ninth images, each under the house prompt with an `Avoid:` list written against that subject. Each was reviewed at full resolution against the image-generation quality gate before entering the repository and each passed on the first attempt. The exact prompts and source paths are recorded in the note's production notes. **One composition defect was found and fixed rather than shipped:** the signature mark on carousel slide 1 overlapped the photograph's lower-right corner and read as an accident; it was moved clear and the slide re-rendered.

**External state changed:** none in this repository's services. Three images were generated in the founder's existing ChatGPT project and downloaded; one Chrome tab was left open on that thread. Nothing published, sent, posted, scheduled, purchased, or configured. No Ghost slot named, no register position taken, no newsletter touched.

**Open, in order:** (1) **Field Note 20 needs a founder read** — it is a complete unit in `drafts/` and can move to `content/` with a register position whenever the founder approves it; until then it has no slot and no authorization. (2) The concurrent session's `substack-*` work is uncommitted in the primary checkout and its `requiredFiles` entries will fail `verify-repository.mjs` there until that session commits; nothing in this branch depends on it, and the two sets of edits to `verify-repository.mjs` touch different regions and should merge cleanly. (3) Carried forward, each re-read rather than copied: the Substack keystroke path is intermittent rather than blocked, and the founder decision — authorize a mechanism or record the slots as manual — is unchanged; Field Note 19 holds register position 18 with no Ghost slot; the corpus test stands at its 2026-08-26 measurement with the bank at nineteen approved pieces against a threshold of twenty-one, so this draft does not bring it due; the distribution problem is a founder call about reach; the newsletter list stands at one member with the A/B suspended under its own marker; the extract-zip acceptances expire 2026-11-09 with their premise re-derived daily.

## 2026-09-12 — Field Note 20 approved and moved into content/ at register position 19

**Client:** Claude Code, same session as the entry above, founder present for this part. **Branch:** `claude/approve-note-20-2026-09-12`, cut from `origin/main` at dba9fb7 after #176 merged at 23:10 UTC. The founder approved "The Opener Doesn't Lift the Door" as written — essay, platform pack, and artwork in one pass, on the day it was drafted.

**What approval changed, and what it deliberately did not.** The essay moved to `content/field-notes/the-opener-doesnt-lift-the-door.md` and the pack to `content/distribution/field-note-20-platforms.md`, both by `git mv` so the history follows. Frontmatter now reads `status: founder-approved` with the approval date and scope; `publication_authorized` stays **false** on both files. **No Ghost slot is named, no newsletter is queued, and nothing is posted.** Approval covers the work, not a slot — publication timing remains a separate founder decision under launch authority.

**Register position 19, appended.** Projected 2026-12-22. Appending moves nothing: no scheduled slot or projection above it changed, and none of the three binding cross-reference constraints involves this note, because the essay references no other note. `scripts/verify-repository.mjs` gained the guard that fails if either file returns to `drafts/`, matching every approved note before it.

**Two run-length facts were recorded in `publication-order.md` rather than left to be discovered later.** Both are stated as founder reorder decisions and neither was acted on unilaterally. **First, the domestic-mechanical opening run is now four.** Positions 16 through 19 open on a water heater, a spirit level on a top plate, a plumbing dead leg, and a garage door. Position 17's entry had recorded a run of two as the tolerance the bank allows; four is longer, and the precedent that entry set was to state the adjacency rather than hide it. **Second, and more consequential: the witness cadence is now exactly regular.** The five witness pieces sit at positions 7, 10, 13, 16 and 19 — an unbroken three-slot interval. The "Why this order" bullet describing two witness pieces "four slots apart so they do not read as a series" was left exactly as written, because it was true when written and this log does not rewrite standing claims; the new subsection states the current positions beside it. A perfectly regular cadence is the shape that bullet was guarding against. Nothing is labelled or announced, so a reader who is not counting sees nothing — but the next witness append would be the sixth in the same rhythm, and it should be made knowing that.

**The corpus test comes due on the next approved piece.** `**Corpus test last measured:**` stands at 2026-08-26, a bank of 15, next due at 21. This approval takes the bank to **20** — nineteen field notes plus Essay 1. The marker is left alone rather than advanced, because recording a measurement that was not made is the failure it exists to prevent. **The twenty-first approved piece brings the measurement due and `verify-repository.mjs` will fail the build until it is made and recorded.** That is the intended behaviour and it is flagged here so it is not a surprise.

**Files changed:** `content/field-notes/the-opener-doesnt-lift-the-door.md` and `content/distribution/field-note-20-platforms.md` (moved from `drafts/`, frontmatter and the first production note updated); `docs/technical/publication-order.md` (row 19 plus the subsection above); `drafts/README.md` (the approval sentence; the directory again holds no active editorial draft); `scripts/render-field-note-20.mjs` (header comment corrected from DRAFT to approved); `scripts/verify-repository.mjs` (the drafts guard); and this log. No asset was re-rendered — the artwork approved is the artwork committed in #176, byte for byte.

**Verification, each gate named and run, all green in the clean worktree:** `node scripts/verify-ghost-theme.mjs` (17 theme files); `pnpm --dir theme install --frozen-lockfile` then `pnpm --dir theme test`; `pnpm --dir theme zip` with `theme/node_modules/.bin/gscan -z --fatal --verbose dist/grown-men-grow.zip`; `node --test 'scripts/test/**/*.test.mjs'` (168 tests, 0 failures); `node scripts/verify-repository.mjs` (663 tracked files — unchanged, because the moves are renames); `bash scripts/verify-svg-xml.sh` (198 SVG files); plus `git diff --check`, `git diff --cached --check`, and a reviewed staged file list. **The content-scoped gates that a draft in `drafts/` never reaches all ran against this note for the first time on this change set and passed:** the publication-order slot check, the per-slide alt-text count, the single `stance` and `closing_addresses_reader` fields with the witness closing rule, the cross-reference scan, and the four-section platform-pack check with Medium tag legality.

**External state changed:** none. Nothing published, sent, posted, scheduled, purchased, or configured. No Ghost slot, no newsletter, no social surface touched.

**Open, in order:** (1) **Field Note 20 is approved but unscheduled**, as are Field Notes 18 and 19 — three approved notes now hold register positions 17, 18 and 19 with no Ghost slot named. That is a founder call under launch authority. (2) **The corpus test falls due on the next approved piece** (bank 20 of 21) and the build will fail until the measurement is made and recorded in the decision log. (3) **The witness cadence question** above is a founder reorder decision, recorded but not acted on. (4) Carried forward, each re-read rather than copied: the Substack keystroke path is intermittent rather than blocked, and the founder decision — authorize a mechanism or record the slots as manual — is unchanged; the distribution problem is a founder call about reach; the newsletter list stands at one member with the A/B suspended under its own marker; the extract-zip acceptances expire 2026-11-09 with their premise re-derived daily.
## 2026-09-12 — A register for what actually went out on Substack, because the log was the only record and it was wrong twice

**Client:** Claude Code (desktop), founder present and directing. **Branch:** `claude/substack-notes-register`, built in a detached worktree — see the concurrency note below.

**The defect, stated as a mechanism rather than a mistake.** The note tasks hand the copy to the founder and finish. The founder posts minutes later. So the handoff entry asserts a terminal outcome — "nothing posted", "external state changed: none" — at a moment when the outcome is still pending, and it merges. That is not an error of care; it is the shape of the task. It produced a false statement in `main` on 2026-09-08 and again on 2026-09-12, and tonight's instance merged as #174 exactly one minute before the note went out. **Both were caught because a person happened to be watching, which is not a mechanism.** This is the same failure `verify-publication-register.mjs` exists for on Ghost, in the same words its own header uses: the only record of what had happened was the thing that was wrong.

**`docs/technical/substack-notes.md` is now that record, and `scripts/verify-substack-notes.mjs` reconciles it against the live public profile.** Substack has no documented API, but the notes feed is public and answers unauthenticated — `api/v1/reader/feed/profile/<id>` returns each note's id, exact UTC timestamp, and **full body text**. The Ghost checker needs a Keychain key and is therefore `cadence:`; this one needs no credential, so it was written as `release:` with a required CI job. **That was wrong, and CI proved it wrong on the first run — see the 403 finding below.**

**It reconciles in both directions, and the second direction is the whole point.** A row claiming `posted` must name a note the feed carries. A row claiming `missed`, or still blank, must have **no** live note carrying its copy. A presence test would have caught neither 2026-09-08 nor 2026-09-12, because in both cases the register's error was claiming *less* than reality. The test file names that case explicitly so a later reader cannot mistake it for symmetry-for-its-own-sake.

**It also checks "posted verbatim" after the fact, which nothing did before.** The live text is compared byte-for-byte against the pack, through `noteCopyForSlug` — the same extractor the note tasks post from, not a second one. A note edited on the platform after posting previously left no trace anywhere in this repository. Tonight's verbatim check happened only because it was done by hand.

**The register was backfilled by derivation, not from memory.** Every live note was matched to its pack by comparing the feed's text against `extractNote()` output across every pack and note number; all six matched exactly one row. Slots come from `publish-timing.md`. The result is ten rows: **six posted, four missed.** The four misses have three distinct causes — the old 09:30 Saturday slot that could not fire (two), a lateness guard correctly refusing a slot 378 minutes gone (one), and the classifier refusal at the composer after a clean handover (one).

**Launch week's two notes carry an em dash, not a slot.** They predate the cadence table governing anything. Writing a plausible-looking time in that column to keep it uniform would have invented the one fact the column exists to carry, and lateness measured against an invented slot is worse than no figure at all. The parser accepts the em dash and skips those rows for lateness; a test holds it.

**Lateness is derived and reported, never failed on.** It is a fact about a human's evening, not a defect. The figures replace prose estimates and are the baseline the Friday analytics task reads: Note 1 of `call-your-friends-before-theres-a-reason` −4 minutes, `a-confession-can-still-be-selfish` +1, `rest-is-not-a-reward` Note 1 **+149** and Note 2 **+16**. The +149 supersedes the "148 minutes" recorded on 2026-09-08, which was floored rather than rounded; the derived figure is the authority from here.

**The checker was proved to fire rather than assumed to.** Two deliberate breakages, each restored: flipping tonight's row back to pending produced `the register records this note as still pending, but c-335779365 carries its approved copy … The register is stale`, and pointing a permalink at a different live note produced the verbatim failure with both character counts. A gate that has never failed implies coverage it does not have.

**An unreachable feed fails closed.** The claim under test is precisely "nothing was posted", so a service that could not be read must never be allowed to confirm it. `reconcileNotes` throws on a non-array `live` rather than reconciling against an empty list, and a test holds that too.

**The CI job was removed, because CI proved it could never pass.** The first run of `Substack notes register` failed with `Substack answered 403 Forbidden for .../public_profile`, while the identical call from this machine succeeds. Substack blocks datacenter egress — the same refusal Medium gives `curl`, already recorded for the Thursday import. **A required status check that can never pass is not a gate, it is a wall across every merge**, and it would have been strictly worse than no check at all. The job was deleted rather than retried, rather than given a browser-shaped user agent to slip the block, and rather than softened to warn instead of fail — the fail-closed behaviour is the correct half and the CI placement was the wrong half. The checker is now `cadence:`, run on the live machine alongside the publication register before the Monday staging decision.

**That also dissolved a blocker this entry originally recorded.** `verify-required-checks.mjs` requires every job running on a pull request to be a required context, so the new job needed a ruleset entry in the same change set — and both the ruleset write and that checker were refused by the Claude Code auto mode classifier, which reads the same API. No substitution was attempted. With no new job there is no new required context, the ruleset is untouched, and `Required checks` passes on its own. The refusals cost nothing in the end, and the ruleset is recorded as unchanged rather than as pending a founder command.

**A concurrency note, because the working tree was shared and this one nearly went wrong.** `gmg-saturday-draft` was live in `/Users/peacock/Projects/grown-men-grow` at the same time, drafting Field Note 20. It had already added `scripts/render-field-note-20.mjs`, two drafts, and — critically — **its own edits to `scripts/verify-repository.mjs`, the same file this work needed to edit.** The shared copy briefly held both sets of changes interleaved. Committing that file from the shared tree would have swept their half-finished Field Note 20 registration into this pull request, where the referenced carousel assets do not exist and CI would fail on work that is not this change set's. Their edits were never staged, never reverted, and never resolved on their behalf. Everything here was instead built in a `git worktree` created from `origin/main`, and the stray copies this session had left in the shared tree were removed afterwards so a later broad `git add` cannot sweep them into someone else's commit.

**Files changed:** `docs/technical/substack-notes.md`, `scripts/lib/substack-notes.mjs`, `scripts/lib/substack-api.mjs`, `scripts/test/substack-notes.test.mjs`, `scripts/verify-substack-notes.mjs` (all new); `scripts/verify-repository.mjs` (all five registered), `AGENTS.md` (the declared `cadence:` gate, its rationale, and the contract paragraph), and this log.

**Verification, each gate named and run:** `verify-ghost-theme.mjs`; `pnpm --dir theme test`; `pnpm --dir theme zip` with `gscan -z --fatal --verbose`; `node --test 'scripts/test/**/*.test.mjs'` — **184 pass, 0 fail**, up from 168; `verify-repository.mjs` over 645 tracked files; `verify-svg-xml.sh`; `verify-substack-notes.mjs` against the live profile; `git diff --check` clean. `verify-required-checks.mjs` **was not run locally** — blocked, as above, and not reported as passing; CI ran it and it passes now that the change set adds no job.

**External state changed:** none. The ruleset write was attempted and refused, so the ruleset is unchanged. Nothing was posted, sent, scheduled, or purchased.

**Open, in order:** (1) **The register is a `cadence:` gate with no CI enforcement, and that is a real limit, not a solved problem.** Nothing forces it to run; it protects the record only when someone runs it, and its natural home is the Monday staging task, which already runs its sibling. A follow-up should add it there. (2) The keystroke path is intermittent — permitted 2026-09-08, refused 2026-09-12 — and the founder decision it feeds is unchanged, but it is now a data question: the register will show, week by week, what the slots actually do. (3) The task file at `~/.claude/scheduled-tasks/gmg-saturday-note/SKILL.md` still states the classifier denial as certain; it lives outside this repository. (4) The note tasks should record their row at handover and let the checker resolve it, rather than asserting a terminal outcome — a follow-up, deliberately not bundled here. (5) Carried forward: Field Note 19 holds register position 18 with no Ghost slot; the witness commission is a founder call, bank nineteen against a threshold of twenty-one; the distribution problem; the newsletter list at one member with the A/B suspended under its own thirty-recipient marker; the extract-zip acceptances expiring 2026-11-09.

## 2026-09-14 — Claude Code: Week 6 staged; the register ran dry a third time and the notes register gained its first pending rows

**Client:** Claude Code (scheduled task `gmg-monday-staging`, founder not present). **Branch:** `claude/monday-staging-2026-09-14`. The task fired at 09:31 EDT against a 09:30 slot — on time, which remains the Monday-morning pattern.

**The hold was checked first and does not fire.** `**Active hold:**` reads `none` and `**Standing commission:**` reads `none`. No death, illness, or crisis is known to this run. The day's news carries a strike on an Iranian cargo vessel off Qeshm Island in the Strait of Hormuz, presidential remarks about staying in Iran and keeping the oil, and a fatal multi-motorcycle crash on Minnesota's North Shore. The test the hold section sets is whether a reader who just saw the news would find the queued post tone-deaf arriving that morning. This week's essay is about a man checking his own heading; it touches no geopolitics, names no one, and asserts no event. The one line worth weighing was *"Nobody ever got criticized at the funeral for working too hard"* — a passing figure of speech against a day carrying a regional traffic death, not a national tragedy, and it reads as the idiom it is. No escalation exists in the replies to be unresolved; the list stands at one member and every surface was silent at the last readout. Ghost answered as `Grown Men Grow` / `grownmengrow.com` and Buffer as organization `6a79550e9f72378c9c0b69a1`, each its own pinned credential, so there is no sign of compromise on either.

**The register ran dry for the third time, so the staging script's network half ran for real again.** `verify-publication-register.mjs` reconciled all twenty rows against Ghost and found five published and nothing scheduled; a direct read agreed, and `findPostBySlug` returned `NONE` for the target slug. `publication-order.md` position 4 — `rest-is-not-a-reward` — published on time at 2026-09-08T12:00:00Z, so position 5, `you-cant-outwork-a-wrong-direction`, was the lowest-numbered note with no Ghost post. **That is also the adjacency the constraints require**: the essay's *"if the last field note landed, the rest"* means the note immediately prior, and `rest-is-not-a-reward` is it.

**The dry run was run first and reported no defects** — 3,369-character body, alt text present, `publish_at` ending in `Z`. Precisely what it proves is the payload; the five Ghost calls were still first run for real afterwards.

**Staged and verified independently of the script's own report.** `node scripts/stage-next-field-note.mjs you-cant-outwork-a-wrong-direction 2026-09-15T12:00:00Z` — 12:00Z is 8:00 AM ET, EDT being UTC-4. A separate read confirms `scheduled` for 2026-09-15T12:00:00.000Z, newsletter `default-newsletter`, `email_segment: all`, visibility public, feature image and alt text set, and a 3,460-character body with four `<h2>` and fifteen `<p>`.

**A verification regex read zero headings on a correct post, and the post was not the thing at fault.** The first independent read counted `<h2>` with a bare-tag pattern and reported `h2 count: 0` against a dry run that had built four. Ghost adds anchor `id` attributes when it renders — `<h2 id="effort-is-the-easy-part">` — so the tag never appears bare, and the same attributes account for the whole 3,369 → 3,460 length delta. An attribute-tolerant pattern found all four. Worth recording because the failure looked exactly like a mangled body on a post scheduled to email everyone, and the instinct it invites is to re-cycle the post rather than to check the check.

**The metadata was diffed against the repository source and all six fields agree.** `buildPostPayload` derives `title`, `custom_excerpt`, `meta_title`, `meta_description`, `email_subject`, and `feature_image_alt`; each matches what Ghost holds. This note carries no `# Metadata` section, so `meta_title` is the derived `You Can’t Outwork a Wrong Direction | Grown Men Grow`. **That is the string Medium's URL importer will take as Thursday's headline**, suffix included — the standing consequence of the derivation, recorded on 2026-08-31 and 2026-09-07 and now a third time, and Thursday's task should expect it.

**The feed-tile check passes both ways.** `verify-repository.mjs`'s photograph-exclusivity gate is green across the corpus, and the live comparison the gate cannot make was run: the most recent Instagram post Buffer has actually **sent** is the Field Note 7 carousel of 2026-09-10, whose tile is built on `hammock-midday-rest.png`. This week's tile is `truck-hood-map.png`. Different photograph, different composition, no grid collision.

**Buffer's queue was empty and now holds four scheduled posts**, mode `customScheduled`, scheduling `automatic`, every one verified by reading the queue back rather than by trusting the mutation's reply. Bluesky fragment Tue 16:00Z; LinkedIn Wed 14:00Z; Instagram carousel Thu 13:00Z with seven slides and seven alt-text strings attached, read back through `assets { ... on ImageAsset { image { altText } } }`; Bluesky canonical link Sat 22:30Z. The seven carousel PNGs were uploaded to Ghost storage as `fn8-c1.png` through `fn8-c7.png`, following the naming the earlier weeks set.

**Every string was read out of the approved files by script rather than retyped**, so "verbatim" is a property of how the copy got there and not a claim about care. The extractor asserts seven alt lines and refuses any string still carrying `[canonical link]`; the one substitution is that placeholder → the live URL, which is the pack's own convention and is visible in every previously sent post.

**The Substack notes register gained its first pending rows, which is the first time it has been used forward rather than backward.** It was built on 2026-09-12 by backfilling ten rows from the live feed. This run added the two rows for this week's slots — Note 1 Tuesday 12:00, Note 2 Saturday 18:30 — with `State` blank, and the checker reconciled all twelve and confirmed no live note carries either note's copy. That blank-row direction is the one that was wrong in `main` twice, so the rows are not decoration: whichever note task runs next has a row to fill, and a claim that nothing was posted is now checkable rather than asserted.

**The Week 6 phone kit is built** at `Week 06 — You Can’t Outwork a Wrong Direction`, in the founder-ruled naming. Three action folders — Substack Note 1 (Tue), Substack Note 2 (Sat), the optional Thursday story link sticker — plus the no-action backup folder holding all four Buffer copies and the seven slides with their alt text. The `READ ME` leads with the fact that nothing changed from last week, and states the keystroke position as the evidence now has it: worked 2026-09-08, refused 2026-09-12, nothing changed in between.

**No new reader-facing prose was written this run.** Everything that reaches a reader is verbatim approved copy. The kit's `READ ME` is operating instructions addressed to the founder, not publication copy, so the nine tests in `editorial-underpinning.md` do not apply to it; they were not run against it and are not claimed to have been.

**Files changed:** `docs/technical/publication-order.md` (position 5 marked scheduled), `docs/technical/substack-notes.md` (two pending rows), and this log. No script or content file was touched. Outside the repository: the iCloud kit.

**Verification, each gate named and run:** `verify-ghost-theme.mjs`, `pnpm --dir theme test`, the theme zip plus `gscan -z --fatal`, `node --test 'scripts/test/**/*.test.mjs'` (184 pass, 0 fail), `verify-repository.mjs`, and `verify-svg-xml.sh` (198 SVGs) — every `gate:` line in `AGENTS.md`'s declared block, executed by explicit path. Both `cadence:` gates were run as this task's own inputs: `verify-publication-register.mjs` (20 rows) and `verify-substack-notes.mjs` (12 rows, re-run after the edit). Plus `git diff --check` and a reviewed staged file list.

**External state changed:** one Ghost post created and scheduled with the newsletter bound; eight images uploaded to Ghost storage (one feature image, seven carousel slides); four Buffer posts created as scheduled. Nothing published, nothing sent, nothing posted to a social surface. The Ghost post sends on Tuesday at 8:00 AM ET under the standing authorization for approved copy at schedule-of-record slots.

**Open, in order:** (1) **Both Substack notes are founder actions again this week** — the keystroke path is intermittent rather than reliably blocked, so each note task should attempt it rather than inherit a prediction, and degrade to the handoff when refused. (2) Whether the Tuesday 12:00 PM fragment slot should stay unattended at all is still a founder decision, unchanged since 2026-09-01, and the notes register now carries the evidence for it: every note that has ever gone out went out with a person at the keyboard. (3) Thursday's Medium import will again take the ` | Grown Men Grow` suffix as the headline — expected, and a hand fix in that task. (4) Task instruction files still drift out of step with the repository and nothing detects it; nothing new was found this run. (5) The witness cadence at positions 7, 10, 13, 16, and 19 is exactly regular, and breaking it is a reorder that belongs to the founder. (6) Carried forward unchanged: the distribution problem, the newsletter list at one member, and the A/B suspended until thirty recipients.

## 2026-09-15 — Claude Code — SMTP2GO retired, outbound identity on Resend

**Scope completed:** migrated both Gmail send-as aliases from SMTP2GO to Resend, verified the path on a delivered message, and removed the SMTP2GO DNS records.

**Files changed:** `docs/technical/email-identity.md` (status, the expired-premise section, a current-configuration section, and a rewritten Acceptance carrying the real headers), `docs/technical/decision-log.md` and this file (appended).

**External state changed:** `grownmengrow.com` created and verified as a Resend domain (id `30627971`); four Resend DNS records added to the Cloudflare zone and the three SMTP2GO CNAMEs (`em790852`, `s790852._domainkey`, `link`) deleted; one test message sent by the founder from `michael@grownmengrow.com`; one probe message sent by the agent to `delivered@resend.dev`. The founder created the `resend-gmg-sending` key and stored it in the Keychain — the agent never handled the value. **Nothing published, nothing sent to a reader.**
...
## 2026-09-16 — Claude Code — Wednesday draft: Field Note 21, "Everything Comes to Him Flat," drafted as a complete unit

**Client:** Claude Code (scheduled task `gmg-wednesday-draft`). **Branch:** `claude/field-note-21-draft-2026-09-16`.

**Backfill came first and found nothing to build.** All nineteen notes in `content/field-notes/` were checked against the complete-unit table: each has a platform pack, a render script, a seven-slide carousel, a feature image, a review sheet, and gate entries, and `verify-repository.mjs` passed before any change. No approved note is blocked on collateral.

**The draft.** An `assignment` piece on welding positions: the shop is right to turn the work so it can be welded flat, a competent man spends twenty years arranging his life the same way, and the joints that do not turn arrive overhead. Cold lap, the bend test, and the structural code's six-month continuity rule carry the argument. 1,240 words in the generic present; no first person, no asserted event. The standing commission is `none`; the subject test fails for witness because practising out of position is a move he has, and the most recent note (Field Note 20) shipped `witness`. `closing_addresses_reader: yes`.

**Collateral, complete.** Platform pack (Medium, Bluesky ×3, LinkedIn, Substack Notes ×3); three photographs generated in the founder's ChatGPT project, continuing the current Editorial Photography Request thread as its tenth to twelfth images, with exact prompts recorded in the note's production notes; `scripts/render-field-note-21.mjs` with a bead mark used by no other note; seven carousel slides; feature image; review sheet; gate entries. One image was accepted with a flag: in `bend-test-strips-bench.png` the first two strips curl slightly at their ends, which a real bent coupon would not do. It is small at carousel size and is named for the founder rather than smoothed over.

**The nine tests, and what came close.** The first full read failed tests 2 and 7 honestly: the draft implied practice always pays, and it had nothing for the man whose overhead joint had already opened. One paragraph was added for both, stating that practice promises nothing and that scrap does not re-weld a joint that has failed. Test 4 came closest after that — the carve-out for a man already out of position all day says what he is short of, which decides a little on his behalf; kept, and named in the report.

**Files changed:** `drafts/field-note-21-everything-comes-to-him-flat.md`, `drafts/field-note-21-platforms.md`, `drafts/README.md`, `scripts/render-field-note-21.mjs`, `scripts/render-review-contact-sheets.mjs`, `scripts/verify-repository.mjs`, `assets/source/editorial/{pipe-positioner-seam-up,vertical-practice-plate-beads,bend-test-strips-bench}.png`, `assets/drafts/instagram/field-note-21-carousel/` (7 SVG/PNG pairs), `assets/drafts/ghost/feature-images/everything-comes-to-him-flat.{svg,png}`, `assets/drafts/review/field-note-21-carousel.png`, and this log.

**External state changed:** three image generations in the founder's ChatGPT project. Nothing published, sent, or posted. The draft is not founder-approved, holds no register position, and names no Ghost slot.
...
## 2026-09-20 — Claude Code — Thursday Medium import: "You Can’t Outwork a Wrong Direction" published, three days after its slot

**Client:** Claude Code (desktop), scheduled task `gmg-thursday-medium`, re-run on the founder's instruction. **Branch:** `claude/thursday-medium-import-2026-09-20`.

**The story is live on Medium:** https://grownmengrow.medium.com/you-cant-outwork-a-wrong-direction-eb2197f13e30

**The slot was missed outright and this is the recovery, not the run.** The Thursday 2026-09-17 07:00 ET import did not execute and left no entry in this log; the work was done at 21:40 ET on 2026-09-20, roughly 86 hours late. Nothing about the piece expired in the interval — the canonical-first rule only sets a floor — but the miss is the scheduler substrate again and it belongs on the record as a miss rather than as a late success.

**Preconditions, each checked rather than assumed.** `latestPublishedPost()` returned `you-cant-outwork-a-wrong-direction`, published 2026-09-15T12:00:00Z — 133.5 hours before the run against a 40-hour minimum. `https://grownmengrow.com/you-cant-outwork-a-wrong-direction/` returned 200. Both Medium surfaces were checked, because the profile feed lists published stories only and a stale import draft is invisible to it: Drafts read "No stories in draft," and Published held exactly the five prior stories — *Rest Is Not a Reward* (Sep 8), *Friendship Has a Maintenance Schedule* (Sep 1), *A Confession Can Still Be Selfish* (Aug 25), *Call Your Friends Before There’s a Reason* (Aug 18), *Strength Has to Grow Up* (Aug 9). That count agrees with the 2026-09-12 entry, so nothing landed between runs and this was not a double-import.

**The importer took the SEO title for the fourth consecutive week.** It imported as *You Can’t Outwork a Wrong Direction | Grown Men Grow* — Medium reads the page `<title>` and Ghost appends the site suffix to the derived `meta_title`. Corrected to the post's `title` field, *You Can’t Outwork a Wrong Direction*, curly apostrophe intact. The 2026-09-08 entry called this a standing consequence of the derivation rather than a divergence to fix, and it held a fourth time.

**Both edits were made by caret read-back and counted BackSpaces, and neither `Home` nor `End` was pressed.** Those keys are document-scoped in Medium's contenteditable and are what destroyed a title on 2026-08-30 and 5,099 characters of body on 2026-09-03. The title: caret verified collapsed at offset 52 inside the title block, 17 BackSpaces, 52 → 35 characters. The `Grown Men Grow 2 min read` line: caret verified collapsed at offset 25 inside that block and the block confirmed to hold exactly that string, 25 BackSpaces to empty it, then one more to remove the emptied block. Block count went 28 → 27 and the dek above it still measured 45 characters afterwards, so nothing merged or was lost. Each step has a blast radius of exactly N characters and cannot reach another block.

**The preview image imported empty this week, and it was checked rather than assumed in either direction.** The task file's unconditional claim held here and did not hold on 2026-09-12; the field is now genuinely conditional and the check is what settles it. "Change preview image" offered the story's single image, it was selected and confirmed, and the field afterwards read an 880×550 crop from `miro.medium.com` with alt *You Can’t Outwork a Wrong Direction*. The live page's `og:image` is the same asset.

**The tag trap fired again, on the same tag, and was caught the same way.** Typing `Men` produced *Mental Health (9.7M)*, *Mental Health Awareness (3K)*, *Men (1.4K)*, *Mental Illness*, *Mentorship* — the exact ordering recorded on 2026-09-03, with the wanted topic third. Three Downs, the highlight confirmed on screen at *Men (1.4K)*, then Return. `Purpose`, `Discipline`, `Masculinity` and `Personal Growth` each ranked their exact match first; each was still confirmed highlighted before Return rather than committed blind. **The committed chips were then read back from the DOM's own remove-labels** — Men, Purpose, Discipline, Masculinity, Personal Growth — because the keystrokes are not evidence of what landed. After publication the live page's tag hrefs read `men`, `purpose`, `discipline`, `masculinity`, `personal-growth`: the pack's five, in the pack's order.

**The canonical was confirmed twice, before and after publication.** Before: Advanced Settings had "This story was originally published elsewhere" checked, and the field's `value` — not its placeholder, which reads `Type the canonical URL...` — was `https://grownmengrow.com/you-cant-outwork-a-wrong-direction/`. After: a `fetch` issued inside the page context returned HTTP 200 and 187,847 bytes of server-delivered source carrying `<link rel="canonical" href="https://grownmengrow.com/you-cant-outwork-a-wrong-direction/"/>`. Run in the browser, never with `curl`, which Medium answers 403 — an unread exit status there turns a refused request into a false pass. The same source confirms the `h1` is the corrected title and that no `Grown Men Grow N min read` line survives in the body.

**The subtitle question from 2026-09-12 does not arise this week.** The importer lifted the dek, *Effort is the easy part. Aim is the argument.*, and that is byte-for-byte the Medium subtitle the pack specifies. Last week the two diverged; here they agree, so no editorial call was needed and none was made.

**No name and no reshare.** The story publishes under the Grown Men Grow profile — story settings show author *Grown Men Grow* / `@grownmengrow` — with no attribution beyond it. The post-publish dialog offered Facebook, LinkedIn, X and Copy link; none was touched and it was dismissed with its close control. Other platforms receive reviewed native copy, never a Medium reshare.

**A note on the working tree, because it changes how the diff should be read.** The shared checkout held a concurrent session's uncommitted work — five modified files under `scripts/` and an untracked Field Note 22 draft. This session did not touch, stage, or commit any of it: the work was done in a separate worktree cut from `origin/main`, so the commit cannot absorb a neighbour's edits and the gates ran against a clean tree rather than against their work in progress.

**Files changed:** this log only.

**External state changed:** **one Medium story published** — title, body, preview image, five tags and canonical as recorded above, at https://grownmengrow.medium.com/you-cant-outwork-a-wrong-direction-eb2197f13e30. No draft was created and deleted. No Ghost write, no send, no Buffer change, no Substack action, no schedule, no purchase. Nothing was reshared to another platform and no account setting was touched.

**Verification, each gate named and run:** results are listed in the pull request body.

**Open, in order:** (1) **Saturday's Substack Note 2 slot, 2026-09-19 18:30, passed with nothing posted.** The cadence gate was re-run rather than copied forward: the register agrees with Substack on all twelve rows against seven live notes, and that row is the one still pending. Whether it goes out late or is recorded `missed` is the founder's call, and it was left blank here rather than decided by a task that does not own it. (2) The Thursday slot itself did not fire; this is the third scheduled slot in six days to need a hand, alongside the Tuesday note and the Saturday note, and the founder question from 2026-09-01 about whether these slots should be scheduled at all now has that much more behind it. (3) The task file's preview-image step is correctly conditional and its tag step correctly states the general rule; both were followed as written and neither needs an edit this run. (4) Carried forward unchanged: the distribution problem, the newsletter list at one member, the A/B suspended under its thirty-recipient marker, the founder calls open in the register and decision log, and the extract-zip acceptances expiring 2026-11-09.
...
## 2026-09-20 — Saturday note stood down; the slot guard was reading the wrong day

**Client:** Claude Code (desktop app), scheduled task `gmg-saturday-note`, fired 21:30 ET Sunday.

**Scope: nothing posted.** No Substack note, no Ghost action, no Buffer change, no external account touched. The preflight returned exit 20 and the run stopped there, as the task contract requires. The lock was taken and released; `~/.claude/scheduled-tasks/.locks/` was verified empty afterwards rather than assumed.

**The slot was missed.** `gmg-saturday-note` did not fire at Saturday 2026-09-19 18:30 ET — the desktop application was closed through the evening — and the catch-up fire arrived 27 hours later. The live profile was read before anything was written: `verify-substack-notes.mjs` reconciles twelve register rows against seven live notes and finds none carrying this note's copy. `docs/technical/substack-notes.md` now records the row `missed`, and its summary is re-dated to today: seven of twelve posted, five missed, four distinct causes.

**Correction to an earlier entry.** The Friday readout states that this register row "was recorded `missed` by a concurrent session while this readout ran." **That edit is not in `main`.** The row was blank at `062bb41` and blank at `3da1f2f`, and it was blank in the working tree when this session read it tonight; no commit between `09f022e` and `3da1f2f` touched it. The claim was true of some session's working tree and never of the repository. Recorded here rather than by amending that entry, per the append-only rule — and it is the same shape as the two failures that caused the register to exist: a log asserting an outcome nothing had verified.

**The defect this session actually fixed.** The run refused itself for the wrong reason. It reported "181 minutes past the 18:30 slot" because `slotEpochMs` resolves a slot on whatever calendar day the run falls on, so it measured Sunday's 18:30; the slot it was serving had been gone 1,621 minutes. The verdict was correct by accident — 21:30 sits past Sunday's grace window. **Had the application opened at 17:00 that Sunday the verdict was `wait`: sleep ninety minutes, post the Saturday note on Sunday evening, and record it as on time.** Demonstrated against the module before any change, and the essay precondition does not catch it, because Sunday is inside the week its Tuesday essay opened. A miss leaves a blank the register can see; a wrong-day post leaves a row that looks correct and moves the baseline the Friday analytics task reads.

**What changed.** `NOTE_TASK_SLOT_WEEKDAYS` in `scripts/lib/note-slot.mjs` binds each note task's slot to its weekday; `slotVerdict` gains `slotWeekday` and refuses a wrong-day run *before* consulting the clock, so such a run can never reach `wait`; `weekdaySlotEpochMs` reports the run's true distance from the slot it was serving. `decide()` gives a wrong-day stand-down its own reason naming both days, and still carries the copy so the note can be handed over. The preflight resolves the weekday from the task name and **fails closed on an unrecognised task** — the weekday previously lived only in each task's cron, inside a `SKILL.md` outside this repository where no gate could read it, which is why the day went unchecked. `verify-repository.mjs` reconciles the table against the schedule-of-record table in `publish-timing.md` in both directions; that check was mutation-tested in both directions and fails as intended.

**Founder decision now due.** `publish-timing.md` committed in advance, on 2026-08-30: if the Saturday note missed from the evening slot as well, "no unattended Saturday slot is reachable on this machine and the note becomes a founder-run action rather than a scheduled one." The evening slot has now missed once. **Nothing was moved, disabled, or rescheduled on this finding** — it is reported, and the decision is the founder's.

**Files changed:** `scripts/lib/note-slot.mjs`, `scripts/lib/note-decision.mjs`, `scripts/note-task-preflight.mjs`, `scripts/verify-repository.mjs`, `scripts/test/note-slot.test.mjs`, `scripts/test/note-decision.test.mjs`, `docs/technical/substack-notes.md`, `docs/technical/publish-timing.md`, `AGENTS.md`, and this log. **External state changed:** none.

**Verification:** all six `gate:` lines run and green — theme, theme tests, zip + GScan (exit 0), `node --test` 200 passing, repository verification, SVG. Both `release:` gates green (`verify-required-checks.mjs`; `verify-dependency-exemptions.mjs` runs in CI). Both `cadence:` gates run on the live machine: the notes register agrees with the live feed on all twelve rows, worst lateness 268 min. The preflight was exercised end-to-end at four pinned instants against a scratch lock dir — wrong-day 20, on-day 0, on-day-early 10, unknown-task 1.
...
## 2026-09-20 — Saturday draft: Field Note 22, "He Writes Both Parts", a complete assignment unit

**Client:** Claude Code (desktop app), `gmg-saturday-draft` scheduled task.

**Scope completed.** The week's second complete field note, drafted end to end: essay, platform pack, three photographs, render script, seven carousel slides, feature image, review sheet, and gate entries in one change set. Nothing is published, sent, posted, or staged.

**Backfill ran first and found nothing owing.** Every one of the twenty approved notes was checked against the complete-unit table in `content-pipeline.md`: packs `field-note-02` through `21`, render scripts `02` through `21`, twenty carousels, twenty review sheets, and a feature image per slug. The backfill clause consumed none of the run.

**Stance is `assignment`, and that was a decision rather than a default.** The 2026-09-16 corpus entry puts `witness` at five in twenty-one, roughly the quarter the Stance section describes, and argues for no further commission; `operating-cadence.md` carries `**Standing commission:** none`, so no commission line is cleared here. The recent run has not tilted back toward obligation — Field Note 20 was `witness` and 21 `assignment` — and the subject test fails for witness anyway, because what a man drafts on the drive home is a move he has. Writing a sixth witness piece into that balance would have been forcing it, which the task instruction forbids in those words.

**Shape, against Wednesday's.** Field Note 21 opened on a welding-shop rule and derived a life from it — concrete and external, `trade`. This one opens on a man rehearsing a speech in a parked vehicle and does not reach a mechanism until its third section, so the `opening` marker is `other`. That is also the placement the register wants: position 20 is a `witness` piece, so an appended `witness` note would fail the adjacency check, and an appended `trade` opening would run 19–20–21 at the three-row ceiling.

**Imagery: four generated, three kept, one rejected on a physical tell.** The first control-cabinet image carried garbled pseudo-lettering across the relay faces and was rejected at full resolution — the same class of tell that rejected an image in #189 — and the replacement prompt turns the lettered faces away from the camera rather than cropping around the failure. Every prompt and source path is recorded in the note's production notes. The lead photograph renders its dashboard band larger than the prompt asked and shows no windshield glass, so the alt text and visual direction describe what the photograph actually shows rather than what was requested.

**Signature mark.** A `contact gap` — a fixed lower contact block and a hinged upper arm, drawn closed and drawn chattering with a spark across the gap and pitting on the lower face. Its first render put the gap geometry wrong; the arithmetic now derives the lifted contact's position from the same rotation the arm group uses, so the spark always spans the real gap.

**Files changed:** `drafts/field-note-22-he-writes-both-parts.md`, `drafts/field-note-22-platforms.md`, `drafts/README.md`, `scripts/render-field-note-22.mjs`, `scripts/render-review-contact-sheets.mjs`, `scripts/verify-repository.mjs`, `assets/source/editorial/{driveway-house-dusk-dash,control-relay-din-rail,relay-contacts-open-bench}.png`, `assets/drafts/instagram/field-note-22-carousel/` (7 PNG/SVG pairs), `assets/drafts/ghost/feature-images/he-writes-both-parts.{png,svg}`, `assets/drafts/review/field-note-22-carousel.png`.

**External state changed:** none. Three prompts were sent in the founder's ChatGPT project, continuing the current Editorial Photography Request thread, and four images generated there.

**Verification:** every `gate:` line in the fleet-gates block run by name — `verify-ghost-theme` (17 theme files), `pnpm --dir theme test` (no fatal issues), the theme zip with `gscan --fatal` (exit 0), `node --test` (191 tests, 0 failures), `verify-repository` (716 tracked files, 26 review sheets, 73 editorial sources), `verify-svg-xml` (214 SVG files), and `git diff --check` clean. No `cadence:` gate was run; none is due to this change set.
...
## 2026-09-21 — Claude Code: Week 7 staged in Ghost; a stale pack flag fixed and gated; Buffer queue awaiting permission

**Client:** Claude Code (scheduled task `gmg-monday-staging`, fired at 15:53 EDT against a 09:30 slot; the founder was present partway through). **Branch:** `claude/monday-staging-2026-09-21`.

**The hold was checked first and does not fire.** `**Active hold:**` and `**Standing commission:**` both read `none`. The day's news is the Russia–Iran sanctions act, renewed US–Iran threats, a Houthi missile intercepted over Riyadh, a North Korean missile test, and the coming Trump–Xi summit. The essay is about rating a man's capacity like a lifting sling. It touches no geopolitics, names no one, and asserts no event, so a reader coming from that news would not find it oblivious. Ghost and Buffer each answered on their pinned credentials, with no sign of compromise.

**Register.** `verify-publication-register.mjs` reconciled 22 rows: 6 published, 0 scheduled. Position 6, `nobody-rigs-to-the-breaking-strength`, was the lowest-numbered note with no Ghost post and is bound by no ordering constraint. `verify-substack-notes.mjs` reconciled 12 rows before this run's edit and 14 after.

**A stale approval flag, found and fixed.** The note's frontmatter records founder approval dated 2026-08-16. Its pack, `content/distribution/field-note-12-platforms.md`, still read `status: draft — NOT founder-approved` / `approved: no`. The decision log's 2026-08-16 entry records the founder approving essay, pack, and artwork together, so the flip was missed in #97. The auto-mode classifier refused the agent's first attempt to flip the flag. The founder then confirmed in chat that everything was already approved and directed the fix. To stop a recurrence, `verify-repository.mjs` now fails any pack under `content/distribution/` whose frontmatter does not read `status: founder-approved`. The check was run before the flip, failed on this file by name, and passed after it. `AGENTS.md` records the rule. The fix covers only the frontmatter: no approved copy was changed.

**Ghost is staged and independently verified.** The dry run was clean: body built, alt text present, `publish_at` in `Z`. Then `node scripts/stage-next-field-note.mjs nobody-rigs-to-the-breaking-strength 2026-09-22T12:00:00Z` ran; 12:00Z is 8:00 AM EDT. A separate read shows `scheduled` for 2026-09-22T12:00:00.000Z, newsletter `default-newsletter`, `email_segment: all`, visibility public, feature image set, and six `<h2>` and 27 `<p>` in the body. **All six metadata fields match `buildPostPayload`'s output:** `title`, `custom_excerpt`, `meta_title`, `meta_description`, `email_subject`, and `feature_image_alt`. `meta_title` is the derived `Nobody Rigs to the Breaking Strength | Grown Men Grow`, and Medium's importer will take that suffix as Thursday's headline, as in prior weeks.

**The feed tiles are distinct.** The photograph-exclusivity gate passes. The most recent Instagram post Buffer has sent is the Field Note 8 carousel of 2026-09-17, whose tile is `fn8-c1.png` (truck-hood map). This week's tile is built on `sling-capacity-tag.png`.

**Buffer is NOT queued.** Buffer had no scheduled posts. The queue script is built and its dry run is clean. It extracts every string by script from the approved pack and note, asserts seven alt lines, and refuses any leftover placeholder. The auto-mode classifier refused the live run three times: twice as a transient classifier error, then as `[External System Writes]`. Nothing was uploaded and no post was created. The four intended posts: Bluesky Post 1 Tue 16:00Z, LinkedIn Wed 14:00Z, Instagram carousel (seven slides uploaded to Ghost storage as `fn12-c1..7.png`, with per-slide alt text) Thu 13:00Z, and Bluesky Post 3 (canonical) Sat 22:30Z.

**Kit.** `Week 07 — Nobody Rigs to the Breaking Strength` is built in the founder-ruled naming: folders 1 (TUE Substack Note 1), 2 (SAT Substack Note 2), 3 (THU story link sticker, with UTM URL), and 9 (Buffer backup copies, the seven slides, the caption, and the alt text). Until the queue lands, the READ ME's first block states that Buffer is not yet queued. The READ ME is operating instructions, not publication copy. No reader-facing prose was written, so the nine editorial tests were not run and are not claimed.

**Files changed:** `content/distribution/field-note-12-platforms.md` (frontmatter only), `scripts/verify-repository.mjs`, `AGENTS.md`, `docs/technical/publication-order.md` (position 6 → scheduled), `docs/technical/substack-notes.md` (two pending rows), and this log.

**External state changed:** one Ghost post created and scheduled with the newsletter bound, and one feature image uploaded. Nothing was published, sent, or posted, and nothing reached Buffer.
