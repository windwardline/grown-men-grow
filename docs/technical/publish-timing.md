...
**Observed 2026-09-20.** `gmg-saturday-note` did not fire at its 6:30 PM ET slot on Saturday 2026-09-19. The desktop application was closed through the evening, and the catch-up fire arrived at 9:30 PM ET on Sunday — 27 hours after the slot. The preflight refused it and no note went out. The live profile was read afterwards rather than assumed: `verify-substack-notes.mjs` reconciles twelve register rows against seven live notes and finds no note carrying this one's copy, so the register now records it `missed`.

**The condition the Saturday section set has been met.** That section committed in advance: "If the Saturday note misses from the evening slot as well, the conclusion is that no unattended Saturday slot is reachable on this machine and the note becomes a founder-run action rather than a scheduled one." The evening slot has now missed once. The evidence is one observation, which is exactly the weight the section assigned it in advance, so the conclusion is reported here and the decision belongs to the founder. **Nothing has been moved or disabled on this finding.**
