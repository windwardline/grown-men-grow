...
**Observed 2026-09-20.** `gmg-saturday-note` did not fire at its 6:30 PM ET slot on Saturday 2026-09-19. The desktop application was closed through the evening, and the catch-up fire arrived at 9:30 PM ET on Sunday — 27 hours after the slot. The preflight refused it, as it should have, and posted nothing.

**The note went out anyway, at 9:53 PM ET that Sunday, because the founder posted it by hand.** `c-341987670`, 1,643 minutes after its slot and verbatim against the approved pack. The register records it `posted` with that lateness rather than `missed`, because it was posted; the lateness column is where the failure stays legible. The reconciliation was run twice against the live feed — before the hand-off, finding seven live notes and none carrying this copy, and after, finding eight and this one among them. Neither state was assumed.

**The condition the Saturday section set has been met, and the hand-post is what meets it.** That section committed in advance: "If the Saturday note misses from the evening slot as well, the conclusion is that no unattended Saturday slot is reachable on this machine and the note becomes a founder-run action rather than a scheduled one." The evening slot did not fire. That the note exists on the profile tonight is not evidence against the conclusion — it is the conclusion, arriving as a founder-run action in everything but the register's own vocabulary. The evidence is one observation, which is exactly the weight the section assigned it in advance, so it is reported here and the decision belongs to the founder. **Nothing has been moved, disabled, or rescheduled on this finding.**

**The separate defect, which is the one worth fixing.** The run stood down for the wrong reason. It reported itself "181 minutes past the 18:30 slot" — measured against *Sunday's* 18:30, because `slotEpochMs` resolved the slot on whatever calendar day the run happened to fall on. The slot it was actually serving had been gone for 1,621 minutes. The verdict was right by accident: 9:30 PM sits past Sunday's grace window. **Had the application opened at 5:00 PM that Sunday, the verdict was `wait` — sleep ninety minutes, post the Saturday note on Sunday evening, and record it as on time.** The essay precondition does not catch it either, because Sunday falls inside the week its Tuesday essay opened.

**Why that mattered more than the miss.** Lateness is reported and never failed on, because it is a fact about a human's evening. A note posted into the wrong day is not that: it moves the baseline the Friday analytics task reads, which is the reason this page's slots are measured at all. A miss leaves a blank the register can see. A wrong-day post leaves a row that looks correct.

**Fixed by binding the slot to its weekday.** `NOTE_TASK_SLOT_WEEKDAYS` in `scripts/lib/note-slot.mjs` states which day each note task's slot belongs to, and `slotVerdict` refuses any run on another day before it consults the clock — a wrong-day run can never reach `wait`. The weekday had lived only in each task's cron, inside a `SKILL.md` outside this repository, where no gate could read it; that is why the day went unchecked. The preflight resolves it from the task name and **fails closed on a task it does not recognise**, because a note task whose day nothing knows is precisely the run this guard exists to refuse.

**Derived, not copied.** `verify-repository.mjs` reconciles that table against the schedule-of-record table above and fails when the two disagree in either direction. A constant copied out of a document expected to change is a premise that goes stale while every check stays green — the same reasoning that derives `DEFAULT_PUBLISH_WEEKDAY` from the Ghost row. The half-move this page warned about when the Saturday slot moved now has one fewer place to hide: the cron and the `--slot` argument still live outside this repository, but the day no longer does.
