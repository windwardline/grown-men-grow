---
title: He Writes Both Parts
slug: he-writes-both-parts
dek: The speech gets better every time he runs it. The other man never gets a line.
byline: none — publication voice (founder ruling 2026-08-10)
access: public
email_subject: He Writes Both Parts
preview: Nobody has ever lost an argument in his own truck.
feature_image_alt: A warm lit house at dusk across a driveway, seen over the dark out-of-focus top of a dashboard from a parked vehicle, beside a close view of an industrial control relay on a rail in an open, well-kept control cabinet, in a paper collage.
status: founder-approved
approved: 2026-09-20 — essay, platform pack, and artwork approved as written on the day they were drafted; approval covers the work, not a slot
publication_authorized: false — no Ghost slot named, no newsletter, no social posting; publication remains separately gated
stance: assignment — the subject test fails for witness, and correctly: what he drafts is a move he has, and the closing hundred words turn to him
closing_addresses_reader: yes — the last paragraph turns to him in the second person and leaves the decision named but unmade
opening: other — opens on a man rehearsing a speech in a parked vehicle, not on a trade or a mechanism; the relay arrives at the third section
personal_claims: none included; add only from founder-supplied facts
artwork_status: complete; founder-approved 2026-09-20 under the per-article image rule and the photograph-exclusivity ruling
origin: written 2026-09-20 by the Saturday draft task with the standing commission at `none`, so no commission line is cleared by this change set. Approved the same day and moved into content/ at register position 21
---

# Ghost essay source

Somewhere between the job and the driveway, a man gives a speech.

It is a good speech. It has been through drafts. The opening is tighter than it was last month, there is a line two thirds of the way in that lands hard enough that he says it out loud in the cab, and the ending has been rewritten three times, because the first two were merely correct and this one is final. He has never lost this argument. In the truck he is undefeated.


The other man is not in the truck. The other man is at a hockey game, or asleep, or four years dead.

## The speech is not stupid

Nobody rehearses for no reason. Something happened that he had no answer for at the time — a line delivered in front of other people, a decision handed down with a smile on it, a sentence from his father carrying thirty years of interest. He was caught flat, and going back over the ground to find where the answer was is the sanest thing his head knows how to do about that.

That is preparation, and preparation is not a character flaw. A man who walks into a hard conversation cold, trusting the right words to show up, finds out that they do not.

The problem is not that he is running the conversation. It is what the conversation turned into while he was not watching.

## Chatter

A relay is a small switch thrown by a small current so a large one does not have to be handled by hand. A coil pulls a set of contacts closed, the contacts carry the real load, and the coil holds them shut for as long as it is fed.

When what feeds that coil is marginal — a long run of undersized wire, a tired battery — something specific happens. The coil pulls in. Closing the contacts puts the load on, the load drags the voltage under what the coil needs to hold, and it drops out. With the load off, the voltage comes back up. The coil pulls in again.

The trade word for it is chatter, which is exactly the sound. Several times a second, for as long as anything is asking it to work.

Nothing in that relay is broken. Every closure is a correct response to the voltage it had at the instant it closed. The loop runs on the fact that closing is the thing that causes the opening.

A man runs the conversation again in order to settle it. Running it again is what keeps it live.

## He casts the other man

A rehearsed argument is undefeated because it is cast. He plays himself, which is fair enough, and he also plays the other man, which is not. He is not a generous casting director: the other man gets the weakest version of his own position, in the tone that is easiest to answer, and he never brings the one fact that would be genuinely inconvenient, because a reasonable opponent ruins the speech.

Run that for a year and two things drift.

The man in his head gets promoted. Every pass raises the stakes slightly, because a speech this good needs somebody worth giving it to, so the offhand remark becomes deliberate and deliberate becomes a man who has been doing this the whole time with his eyes open. That is not lying. It is what happens to any story told two hundred times by someone with a stake in it.

And he gets very good at answering a man who does not exist. The real one, if he ever turns up, says something off-script inside ten seconds, every rehearsed line goes in the bin, and there he is with his mouth open — the exact thing the rehearsal was built to prevent.

## The arc is small

Every time a relay makes and breaks, a small arc jumps the gap. One is nothing — a spark you would want a dark room to see. But the faces pit, the pitting makes the next arc bigger, and two pieces of silver built to touch ten million times end up welded shut or burned past passing current. The device is not killed by the load. It is killed by the cycling.

The cost is not the hours, although a man who added those up honestly might want to be sitting down. It is that the body has no category for rehearsal. It runs the same chemistry for the imaginary version it ran for the real one — jaw, pulse, the sentence arriving with heat already on it. So a man comes through his own front door having just been insulted by nobody, and the people in the kitchen get someone walking out of a fight none of them attended, with no idea why the question about the dishwasher went the way it did.

He is not being dishonest with them. He is just not all the way back.

## Two things that look identical from inside

There is a real distinction here and it does not resolve into advice.

Some of these conversations should happen. The words are owed, the man is reachable, and the rehearsal is the only preparation anyone is going to give him. Telling him to drop it is telling him to let the thing stand, and there is no interest here in the peace where one man agrees to be the only one who never says what happened.

Some of them cannot happen. The man is dead, or he is alive and holds a different memory of it, and would meet the finest speech ever written with a look of mild confusion, which is worse than an argument.

The tell is not the subject. It is what is getting drafted. If the work is on what he would say, that is preparation, and it keeps its value whether or not the day comes. If the work has moved on to what the other man would finally admit — the face, the pause before he said it — it became a verdict a while ago. A verdict is the one document a man cannot issue on his own behalf, and writing it out in somebody else's voice does not get it issued.

And some men are carrying something none of this is sized for. The thing in the rehearsal is not a remark in a meeting; it is a childhood, or a son who has not called in two years. A field note is not for that, and a man holding it does not need a paragraph from a stranger about relay contacts. He needs somebody in the room with him over a long stretch of time, paid to know what they are doing or attached to him enough to stay.

## What he is actually asking for

Strip the speech down and it is usually not asking for an apology. Underneath is something smaller and harder to get: he wants it on the record that the thing happened, and that it was not nothing.

That is a reasonable thing to want. It is also a thing another person has to say, which means it may not be available — not because he handled it badly or did not want it enough, but because the only man who could put it on the record has no intention of doing so.

What he does about that is genuinely his. Some men say it out loud to the man's face, get nothing back, and are still glad they said it. Some put it down, and they tend not to call that forgiveness and tend not to care what it gets called.

## The supply side

A chattering relay is not fixed at the relay. Bigger wire, a better source, something that can hold the voltage up once the load lands. Everyone who has chased one has spent an afternoon staring at the buzzing part first, because that is the part making the noise.

You already know which argument it is. It has a running time and a best line, and you could start it right now from any point. The only part of it you were ever able to cast is your own, and whether it ever gets said out loud to the man it was written for is a separate question, and yours.

# Instagram carousel source

## Slide 1

HE WRITES

BOTH PARTS.

The speech gets better every time he runs it.

## Slide 2

Somewhere between the job and the driveway, a man gives a speech.

He has never lost this argument. In the truck he is undefeated.

## Slide 3

Nobody rehearses for no reason.

He was caught flat once, and his head is going back over the ground to find where the answer was.

## Slide 4

Chatter. The coil pulls in, the load drops the voltage, the coil lets go, the voltage comes back.

Closing is the thing that causes the opening.

## Slide 5

He plays himself, which is fair enough, and he also plays the other man, which is not.

He never brings the one fact that would be genuinely inconvenient.

## Slide 6

Every make and break draws a small arc. One is nothing. The faces pit.

The device is not killed by the load. It is killed by the cycling.

## Slide 7

The body has no category for rehearsal.

He comes through his own front door having just been insulted by nobody.

# Instagram caption source

Somewhere between the job and the driveway, a man gives a speech. It has been through drafts. The opening is tighter than it was last month, the ending has been rewritten three times, and he has never lost this argument. The other man is not in the truck.

Nobody rehearses for no reason. Something happened he had no answer for at the time, and going back over the ground to find where the answer was is the sanest thing his head knows how to do about it. That part is preparation, and preparation is not a character flaw.

The trouble is what the conversation turned into while he was not watching. He plays himself, which is fair enough, and he also plays the other man, which is not — and the other man never gets the one fact that would be genuinely inconvenient, because a reasonable opponent ruins the speech.

Run it for a year and he is very good at answering somebody who does not exist.

# Instagram alt text source

- Slide 1: Title He Writes Both Parts above a photograph of a warm lit house at dusk across a driveway, seen over the dark out-of-focus top of a dashboard from a parked vehicle, and a small drawn mark of two switch contacts closed together.
- Slide 2: Serif text over a torn oxblood field about a man giving a speech between the job and the driveway, with the closed contact mark drawn in pale ink.
- Slide 3: Sparse page about nobody rehearsing for no reason, the closed contact mark drawn at full scale on a clean joint.
- Slide 4: The contact mark drawn at full scale in its chattering state, the upper arm lifted off the lower face with a jagged spark crossing the gap, above text explaining relay chatter.
- Slide 5: Halftone field beside text about a man casting both parts of his own argument, with the chattering contact mark small.
- Slide 6: A close photograph of an industrial control relay mounted on a rail inside an open, well-kept control cabinet, above text about the small arc drawn at every make and break.
- Slide 7: A pair of worn silver contact faces from a dismantled relay lying on a clean workbench beside a small screwdriver, above closing text about the body having no category for rehearsal.

# Visual direction

- Family: Field Note; image-led cover, alternating photograph and type interiors, warm domestic dusk against clean daylit control-panel detail. Signature: a hand-drawn **contact gap** — a fixed lower contact block and a hinged upper arm carrying the upper contact. Its closed state sets the two faces flush on one clean line; its chattering state lifts the arm a few degrees, puts a jagged spark across the gap, and scatters small pits along the lower face. Same two parts, same pivot, and the only thing that changes is whether they are touching. No prior vocabularies: not the counterbalance, load path, tolerance stack, downspout, strength curve, sling, transfer switch, battery gauge, route line, tally, ledger, alarm arc, diverging fork, anode section, bubble vial, branch stub or bead mark.
- Lead: a warm lit house at dusk across a driveway, seen over the dark out-of-focus top of a dashboard from a parked vehicle.
- Details: an industrial control relay on a rail inside an open, well-kept control cabinet; a pair of worn silver contact faces from a dismantled relay on a clean workbench beside a small screwdriver.
- All three photographs newly generated for this note under the house prompt in `docs/technical/content-pipeline.md`, unique to it, and each used in at most one published asset per the 2026-08-16 exclusivity ruling.

# Production notes

- Written and approved 2026-09-20 — essay, pack, and artwork, on the day it was drafted. Register position 21. No Ghost slot named; publication timing remains a later founder decision.
- Stance is `assignment`. `operating-cadence.md` carries `**Standing commission:** none`, so no commission line is cleared by this change set. The subject test fails for witness and correctly: what he drafts on the drive home is a move he has, and the closing turns to him.
- The corpus reading of 2026-09-16 put `witness` at five pieces in twenty-one, roughly the quarter the Stance section describes, and argued for no further commission. The recent run has not tilted back toward obligation — the last note approved before this one was `assignment` and the one before it `witness` — so writing this as a witness piece would have been forcing it.
- Placed at register position 21. Position 20 is `the-opener-doesnt-lift-the-door`, a `witness` piece, so an appended `witness` note would have sat adjacent to it and failed the spacing check. This note is `assignment` with an `other` opening, which also ends the trade pair at 19–20 rather than extending it to three.
- No first-person anecdote appears and no event is asserted as having happened; the essay runs in the generic present per the 2026-08-08 founder ruling, reaffirmed 2026-08-12.
- The electrical specifics are ordinary control-circuit practice — a relay coil and its contacts, chatter under marginal coil voltage, arcing at every make and break, and contacts that pit and eventually weld or fail to pass current — used as metaphor, not as instruction.
- Photography generated 2026-09-20 in the founder's ChatGPT project, continuing the current Editorial Photography Request thread. Four images were generated and three kept; the rejection was made against the quality gate at full resolution, on a physical tell, not on taste.
- Rejected, and why: the first control-cabinet image carried garbled pseudo-lettering across the front faces of the relays, which is the same class of tell that rejected an image in #189. The replacement prompt turns the lettered faces away from the camera and forbids markings of any kind, which is the pipeline's rule of rewriting a failing mechanism out of frame rather than cropping around it.
- `assets/source/editorial/driveway-house-dusk-dash.png` (slide 1, feature image) — "Now a nineteenth, separate image in the same editorial photography style, portrait orientation 2:3, single image. The view from the driver's seat of a parked pickup truck through a clean windshield at dusk: a well-kept single-family house twenty feet ahead across a tidy driveway, two ground-floor windows and a porch light glowing warm yellow, the sky above it still deep blue with the last of the daylight; the top of the dark dashboard reads as a soft, out-of-focus band across the bottom eighth of the frame and nothing else of the interior is visible. The image suggests a man sitting in his own driveway a minute longer than he needs to, with somewhere good to go inside. A maintained suburban street in ordinary use, trimmed shrubs, a clean walkway. No people, no animals. Contemporary editorial photography with the polish of a major men's magazine, natural texture, restrained fine film grain, candid rather than posed. Dynamic off-center composition, the house set left of center, clean negative space of blue sky above. Warm window light against cool blue evening sky, optimistic, grounded, clearly legible rather than dark. Adult subject, pristine natural anatomy, maintained welcoming setting, no logos, no readable text, no house numbers, no licence plates, no watermark. Avoid: a steering wheel, wiper blades, a rear-view mirror or any interior trim beyond the dashboard band; double or ghosted reflections in the glass; smeared or warped window frames; a house whose windows do not line up with its storeys; a driveway that melts into the lawn; rain streaks; cinematic blue-orange grading; heavy moody darkness; silhouetted figures in the windows; clutter." The dashboard band renders larger than the requested eighth and no windshield glass is visible, so the alt text and visual direction describe the house seen over a dashboard rather than through a windshield.
- `assets/source/editorial/control-relay-din-rail.png` (slide 6, feature image) — "Now a twenty-first, separate image in the same editorial photography style, portrait orientation 2:3, single image. A tight close view of a single grey control relay clipped to a mounting rail inside a clean control enclosure, photographed from below and to one side so the front face of the relay is turned away from the camera and is not visible at all; what fills the frame is the moulded side of the relay body, the rail it is clipped to, and four or five coloured wires leaving its lower terminals in tidy parallel runs and dropping out of frame, each wire clearly seated in a terminal. Every plastic surface in the frame is smooth, blank, unmarked moulding: no printing, no engraving, no symbols, no numbers, no lettering anywhere on any component. The image suggests a small device that switches something much larger than itself. A clean, well-kept machine room in daily use, softly out of focus behind, bright even daylight. No people. Contemporary editorial photography with the polish of a major men's magazine, natural texture, restrained fine film grain, candid rather than posed. Asymmetric composition, the relay filling the lower right, clean negative space upper left. Bright soft daylight, warm, optimistic, grounded. Adult subject, pristine natural anatomy, maintained welcoming setting, no logos, no readable text, no watermark. Avoid: any front face of a relay or contactor; any printed, stamped, engraved or embossed marking of any kind, including illegible pseudo-lettering; wires that end in mid-air or pass through solid parts; tangled or crossing wires; components that merge into each other or into the rail; a rail that bends or floats; sparks, arcing, glowing parts; grime, rust, scorch marks; dramatic moody lighting; heavy bokeh; clutter."
- `assets/source/editorial/relay-contacts-open-bench.png` (slide 7) — "Now a twenty-second, separate image in the same editorial photography style, portrait orientation 2:3, single image. A close overhead-angled view of one small control relay lying on its side on a clean steel workbench with its outer cover removed and out of frame, so the works are open to the camera: the copper coil wound on its core, the thin flat spring arm above it, and at the end of that arm the two small round silver contact faces that meet when it closes, one face bright and smooth and the other dull grey and finely pitted from use. The image suggests a very small pair of surfaces that have been touching and parting for years. A tidy, well-used workshop bench softly out of focus behind, soft daylight from a side window. No people, no hands, no tools in the frame. Contemporary editorial photography with the polish of a major men's magazine, natural texture, restrained fine film grain, candid rather than posed. Dynamic off-center composition, the relay in the lower left third, clean negative space upper right. Soft bright daylight, warm, optimistic, grounded. Adult subject, pristine natural anatomy, maintained welcoming setting, no logos, no readable text, no watermark. Avoid: coins or anything resembling a coin; any printing, stamping, engraving, embossing, symbols, numbers or illegible pseudo-lettering on any surface; clear or transparent plastic; screwdrivers, tweezers, pliers or any tool; a coil whose winding direction reverses or whose wire disappears into the core; a spring arm attached to nothing; duplicated or cloned parts; sparks, arcing, heat glow; rust, grime, burn marks; dramatic moody lighting; heavy bokeh; clutter."
