...
release: node scripts/verify-substack-notes.mjs
...
`verify-substack-notes.mjs` is `release:` for the same reason — it reads the
public Substack notes feed — and CI runs it as the `Substack notes register`
job. It needs no credential, unlike the Ghost register's checker, because the
notes are public; that is why this one can gate a pull request and that one
cannot.
