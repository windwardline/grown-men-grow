...
CI repeats the Ghost theme, repository, and SVG verification. The repository check also enforces feed-tile distinctness (added 2026-08-16 after a repeat reached the Instagram grid): no two grid-facing assets may place the same source photograph in effectively the same frame. Known pairs may be recorded in `KNOWN_TILE_REPEATS` with a stated reason, one pair per entry; it is currently empty and an entry there means a repeat somebody decided to live with, not one somebody fixed. A second rule covers the case the first one missed: a single-asset 1080x1350 feed post must carry no photograph, or one that appears in no other asset, because a one-image post has no sequence to carry it. Carousels, stories, and Reel covers are exempt by design. A third rule carries the founder's 2026-08-16 exclusivity ruling: a photograph belongs to at most one published Instagram asset, with the four posted Essay 1 launch families grandfathered against each other only — any unpublished asset sharing a photograph with anything else fails. Security workflows run Semgrep, secret scanning, and an OSV dependency scan of `theme/pnpm-lock.yaml` that fails on any known vulnerability. Accepted risks live in the root `osv-scanner.toml`, each with a stated rationale and an expiry date; the file sits at the root rather than beside the lockfile because every tracked file under `theme/` ships inside the published theme zip. One entry is live: extract-zip 2.0.1 (GHSA-jmr9-qjv8-65gv), expiring 2026-11-09, at which point the gate fails again and the risk must be re-accepted on that day's facts. An advisory Claude review runs on every same-repo PR via `claude-review.yml`, which deliberately calls the fleet reusable at `@main` — one merge updates every repo. It activates only when the `CLAUDE_CODE_OAUTH_TOKEN` secret is present — reviews bill the owner's Claude subscription, not Console credits; fork PRs never receive secrets, so they skip it by security design. The unused root `vercel.json` is retained solely because it is a fleet-conformance artifact; it is not the publication's hosting configuration.
